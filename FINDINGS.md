...
- **[CAND-F] Ρίζα repo = χωματερή.** ~150 certificate PDFs στο root (emitter γράφει στο CWD), 8× scratch.py, test outputs, daemon logs, README.md 0 bytes, κενά `server`/`index`, φάκελος creator-os ΜΕΣΑ στο creator-os. Θέλει: tracked/untracked απογραφή, emitter fix (output dir), hygiene pass. Trigger: επόμενο νοικοκύρεμα.
