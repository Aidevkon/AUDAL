...
- **[F-111] Δύο ορισμοί του `cepstral_flux`, όχι δύο αναγνώστες — και κάθε σημερινή μέτρηση της βάρδιας γι' αυτό το μέγεθος έγινε στον ορισμό που η παραγωγή δεν τρέχει.** Component: `sp314-dsp/src/analysis/scout_scanner.rs` (`scan_file`, γρ.51 — νέος `MfccAnalyzer` ανά παράθυρο) · `sp314-orchestrator/src/trunk_pass.rs` (`run_trunk_internal`, γρ.659-666/820-845 — κυλιόμενος μέσος όρος σε όλο το αρχείο) · `research/encoder-gap-speech/src/bin/io_layer_diag.rs` (νέο εργαλείο, αναπαράγει την παραθύρωση+MFCC-ring του trunk_pass.rs γραμμή-προς-γραμμή, χωρίς να το τροποποιεί ή να το καλεί). ΜΕΤΡΗΜΕΝΟ 2026-09-16, εννιά αρχεία.
  ...
  **(Γ) Η αιτία — δύο ορισμοί, όχι δύο αναγνώστες.** `scan_file` (`scout_scanner.rs:51`): `let mut mfcc_analyzer = lineos_corpus::mfcc::MfccAnalyzer::new();` — νέος αναλυτής ΑΝΑ ΠΑΡΑΘΥΡΟ, `compute_cepstral_flux` παίρνει τον απλό μέσο όρο ΜΟΝΟ των ~469 MFCC πλαισίων του ΙΔΙΟΥ παραθύρου, μηδέν ιστορικό. `run_trunk_internal` (`trunk_pass.rs:659-666,820-845`): `mfcc_prev: Option<[f32;13]>` και `flux_distances: VecDeque<f32>` (χωρητικότητα 467) — ΜΙΑ instance για ΟΛΟ το αρχείο, `mfcc_prev = Some(mfcc_curr);` (γρ.843) εκτελείται σε κάθε κλήση και ΔΕΝ μηδενίζεται ποτέ· κάθε παράθυρο διαβάζει `flux_running_sum / flux_distances.len()` (γρ.855) — στιγμιαία τιμή ενός κυλιόμενου μέσου όρου έως 466 αποστάσεων που κουβαλάει ιστορικό από πριν το τρέχον παράθυρο. Δεν άλλαξε το I/O· άλλαξε ο ορισμός του μεγέθους.
