...
- **[F-126] Η εξαγωγή της μηχανής από τους χειριστές είναι μετακόμιση αρχείων, όχι ξεμπλέξιμο — η μηχανή είναι ήδη καθαρή εκεί που ζει.** Component: `lineos/m0/m0-daemon/src/handlers/{export.rs,decode.rs,deliver.rs}` · `lineos/m0/m0-daemon/src/domain/dsp_pipeline.rs` · `lineos/m0/m0-daemon/src/agents/executor.rs` · `lineos/m0/m0-daemon/Cargo.toml`.
  ...
  **ΔΙΟΡΘΩΣΗ 2026-09-21 (recon, ίδιο γραπτό κριτήριο, ίδιο αρχείο, ξαναμέτρημα):** το κριτήριο του Σκέλους 2/3 πιο πάνω («παίρνει/επιστρέφει τύπο axum; αγγίζει βάση;») ξαναεφαρμόστηκε στο `handlers/deliver.rs` και έδωσε **4 ΣΗΜΑ / 2 ΔΙΑΚΟΜΙΣΤΗ / 0 ΕΝΔΙΑΜΕΣΕΣ**, όχι 1/2/3. Η διαφορά είναι τρεις συναρτήσεις — `sanitize_title` (`:67-90`), `validate_and_plan` (`:92-224`), `build_minimal_blob` (`:335-369`) — και οι τρεις απαντούν ΟΧΙ και στις δύο ερωτήσεις του κριτηρίου, άρα περνάνε το τεστ γραμμή προς γραμμή. Καμία δεν αγγίζει δείγμα ήχου: η πρώτη καθαρίζει συμβολοσειρά, η δεύτερη φτιάχνει σχέδιο παράδοσης, η τρίτη κατασκευάζει δομή.

  Το εύρημα δεν είναι ότι ένα από τα δύο νούμερα (1/2/3 ή 4/2/0) είναι λάθος. Είναι ότι το γραπτό κριτήριο ορίζει ρητά μόνο το «ΔΙΑΚΟΜΙΣΤΗΣ» (axum-τύπος + db-κλήση) και αφήνει το «ΣΗΜΑ» και το «ΕΝΔΙΑΜΕΣΟ» να μοιραστούν ό,τι περισσεύει χωρίς δεύτερο γραπτό όριο ανάμεσά τους — δύο αναγνώστες που το εφάρμοσαν πιστά πήραν διαφορετική απάντηση στο ίδιο ερώτημα, στο ίδιο αρχείο, στην ίδια μέρα διαφοράς.

  Τι ΔΕΝ αλλάζει: το συμπέρασμα του F-126 στέκει με και τις δύο αναγνώσεις — οι δύο χειριστές (`post_deliver_plan`, `post_deliver`) είναι οι ίδιοι και στις δύο, και η εξαγωγή παραμένει μετακόμιση αρχείων, όχι ξεμπλέξιμο συναρτήσεων. Το πού πέφτει η γραμμή ανάμεσα σε ΣΗΜΑ και ΕΝΔΙΑΜΕΣΟ δεν το αλλάζει.
...
- **[F-133] Οι δύο τελευταίες συναρτήσεις της μηχανής που ζουν στον διακομιστή είναι εικοσιπέντε δεσμοί με τον server-κώδικα, και οι εικοσιπέντε είναι καθαρές — έντεκα διαβατήριο, δεκατέσσερις γνήσιες τοπικές.** Component: `lineos/m0/m0-daemon/src/agents/executor.rs` · `lineos/m0/m0-daemon/src/handlers/deliver.rs` · `lineos/m0/m0-daemon/src/{app_state.rs,spool.rs,blob_store.rs}` · `lineos/m0/m0-daemon/src/domain/{dsp_pipeline.rs,content_type.rs,nodes/{scout_node.rs,dsp_node.rs,certificate_node.rs}}` · `lineos/m0/m0-daemon/src/dsp/{input_lufs.rs,lazy_reader.rs,stream_core.rs,wav_to_raw.rs,orchestrator/nmf_worker.rs}` · `lineos/m0/m0-daemon/src/handlers/timeline.rs`.

  ΜΕΤΡΗΜΕΝΟ 2026-09-21 (recon, δύο περάσματα), ως προϋπόθεση της μετακόμισης που ζητάει το PRD:465: `execute_streaming_plan` και `run_deliver_core` ΕΞΩ από το `m0d` — έξι κατηγορίες του κλαδέματος περιμένουν να φύγει ο διακομιστής μαζί τους.

  **Εικοσιπέντε διαδρομές, εικοσιπέντε καθαρές.** `execute_streaming_plan` (`agents/executor.rs:230-733`) έχει 21 διαφορετικές διαδρομές `crate::` — στην υπογραφή και στο σώμα — και και οι 21 έχουν μηδέν τύπο axum, μηδέν άγγιγμα βάσης, ίδιο και σε βάθος ενός επιπέδου. `run_deliver_core` (`handlers/deliver.rs:375-615`) έχει 4, όλες μέσα στο `blob_store::`, και οι 4 καθαρές. Ο φόβος ότι η συνάρτηση φτάνει βαθιά στον διακομιστή πέφτει — φτάνει βαθιά στη μηχανή που τυχαίνει να ζει εκεί.

  **Και οι μισές είναι διαβατήριο.** Έντεκα από τις εικοσιπέντε είναι επανεξαγωγές: `pub use lineos_types::certificate::{...}` στο `blob_store.rs:37,882,891` (`DeliveryCheck`, `CorrectionRecord`, `NamedValue`, `StoredBlobV2`) · `pub use lineos_types::analysis::ANALYSIS_SAMPLE_RATE as TARGET_SR` στο `dsp/stream_core.rs:12` · `pub use aether_bridge::ContentType` στο `domain/content_type.rs:4`. Οι τύποι ζουν ήδη στη μηχανή· οι γραμμές που τους δείχνουν περνάνε από τον διακομιστή. Είναι η δουλειά των τεσσάρων μετακομίσεων της 20/09 (F-126 κ.ε.), ορατή από την άλλη πλευρά: ο δεσμός είναι ονοματολογία, όχι εξάρτηση.

  **Και οι άλλες δεκατέσσερις ταξιδεύουν μαζί.** Γνήσιες τοπικές του `m0d`, όλες καθαρές. Οι μεγαλύτερες: `run_streaming` (`domain/nodes/certificate_node.rs:219`, 110 γρ.) · `build_intent_and_config` (`domain/nodes/dsp_node.rs:38`, 94 γρ.) · `DeliveryVerdict::compose` (`blob_store.rs:81`, 56 γρ.) · `nmf_worker::spawn` + `::dispatch_all_jobs` (`dsp/orchestrator/nmf_worker.rs:15,72`, 22+23=45 γρ.) · `scout_node::run` (`domain/nodes/scout_node.rs:14`, 37 γρ.) · `pass0_decode_to_dump` (`dsp/input_lufs.rs:104`, 35 γρ.) · `find_sidecar` (`blob_store.rs:593`, 24 γρ.) · `read_scout_sample` (`dsp/lazy_reader.rs:328`, 19 γρ.) · `wav_to_raw_measured` (`dsp/wav_to_raw.rs:55`, 11 γρ.) · `write_sidecar` (`blob_store.rs:461`, 9 γρ.). ⚠ Με τις δύο συναρτήσεις (504 + 241 γραμμές), το σύνολο είναι περίπου 1.240 γραμμές σε δώδεκα αρχεία του `domain/` και του `dsp/` — αριθμητική από τον πίνακα του recon, όχι ξεχωριστή μέτρηση.

  **Η πρόοδος είναι καθαρός τύπος.** Η υπογραφή του `execute_streaming_plan` παίρνει `Option<tokio::sync::broadcast::Sender<crate::app_state::MasteringProgress>>`. Το `MasteringProgress` (`app_state.rs:25`) έχει έξι πεδία — `String`, `u64`, `Option<String>`, `Option<f32>` — με μόνο `#[derive(Debug, Clone, serde::Serialize)]`: μηδέν axum, μηδέν βάση. Ίδιο σχήμα με το `DecodeError`: ο τύπος καθαρός, το σπίτι όχι — ⚠ το ίδιο αρχείο κρατάει και το `AppState` με `DbConn` μέσα (`app_state.rs:85`)· η γειτονιά δεν είναι δεσμός. Ένα μόνο σημείο αποστολής σε όλη τη συνάρτηση (`:273-282`), όχι τρία όπως έγραφε το F-126 — εκείνο μετρούσε γραμμές παραπομπής, όχι κλήσεις.

  **Δύο σημεία που η μετακόμιση θα συναντήσει, καταγραφή όχι κρίση.** Το `run_deliver_core` καλεί `find_sidecar` και `write_sidecar`, και το δεύτερο γράφει στον δίσκο — το F-129 μέτρησε την υπογραφή και το παράπλευρο του `blob_store` ως δουλειά που ΜΕΝΕΙ εκεί· η γραμμή της παράδοσης και η γραμμή της αποθήκευσης τέμνονται στις δύο αυτές συναρτήσεις. Και οι καλούντες: `execute_streaming_plan` έχει δύο σε `src/`, μηδέν σε `tests/`, δεκαεπτά σε `research/`· `run_deliver_core` έναν σε `src/`, πέντε σε `tests/`, έναν σε `research/`. Η μετακόμιση αλλάζει διαδρομή σε εικοσιέξι σημεία κλήσης.

- **[F-134] Το πρώτο πλήρες τρέξιμο του suite: πέντε κόκκινοι, όλοι στο `sp314-dsp`, κανένας δεν αγγίζει τη δουλειά της συνεδρίας.** Component: `lineos/m1/sp314-dsp/src/analysis/scout_scanner.rs` · `lineos/m1/sp314-dsp/tests/{superflux_oracle.rs,w_speech_fit.rs,spectral_flux_contract.rs,adversarial_drone.rs,pass1_integration.rs}` · `lineos/m1/sp314-dsp/src/stft/two_pass.rs` (`use_drums`, `nmfd_k`, :809,:817) · `research/w-speech/{download_librispeech.py,w_speech_v1_params.json}` · `.github/workflows/ci.yml` (F-121).

  ΜΕΤΡΗΜΕΝΟ 21/09 στο μηχάνημα του χρήστη, `cargo test --workspace --no-fail-fast` — το πρώτο πλήρες πέρασμα του suite σε όλη τη συνεδρία· οι κλίμακες μέχρι εδώ σταματούσαν στο `cargo check --tests`, που επιβεβαιώνει μεταγλώττιση, όχι πέρασμα.

  **Το αποτέλεσμα: πέντε στόχοι απέτυχαν, όλοι στο `sp314-dsp`. Το `m0d` πέρασε καθαρό, με τα δοκίμιά του.** Οι επτά αλλαγές της συνεδρίας — τέσσερις μετακομίσεις τύπων (F-126 κ.ε.) και τρία κλαδέματα (Tauri/cockpit, `caddy_running`, τρεις νεκροί κόμβοι DSP) — κρατάνε· καμία από τις πέντε αποτυχίες δεν τις αγγίζει.

  **Τρεις λείπουν δείγματα, δύο ρίζες, όχι ίδια ποιότητα.** `flight_clips_stereo/` — δέκα `.wav` στη ρίζα ενός παλιότερου αποθετηρίου, αδήλωτα στο git (απόντα από το `.gitignore` όπως και από το δέντρο), απόντα από το AUDAL (`find` σε όλο το δέντρο: μηδέν αποτέλεσμα) — σπάει το `test_scan_file_sanity` (`scout_scanner.rs:105-108`, ζητά `clip_transition_st.wav`) και δύο από τα τρία clip του `superflux_oracle.rs` (`:49,71-74`: `clip_speech.wav`, `clip_podcast_st.wav`). Η ίδια εξάρτηση ήταν ήδη γνωστή από τις 21/08 σε άλλο μέτωπο: `tests/pass1_integration.rs:99` έχει ήδη `#[ignore = "Requires local uncommitted flight clips in flight_clips_stereo/"]` και καλεί την ίδια `scan_file` (`:2,48`) — το εύρος εκείνης της πρώτης παρτίδας ήταν `tests/`, το δοκίμιο μέσα στο `src/` (`scout_scanner.rs`) δεν το είδε κανείς. `research/w-speech/corpus` — απών φάκελος, σπάει το `w_speech_fit.rs` (`:39-49`, ζητά `research/w-speech/corpus`) — αλλά ανακτήσιμο: `research/w-speech/` κρατάει τον κατεβαστή (`download_librispeech.py`) και `w_speech_v1_params.json` με 30 εγγραφές `corpus_manifest` (μετρημένο, `python3 -c "import json..."`) και `corpus_source: http://www.openslr.org/resources/12/dev-clean.tar.gz`. Το δεύτερο είναι το τίμιο σχήμα — δεν κουβαλάει το σώμα, κουβαλάει τον τρόπο να το φέρεις και να το επαληθεύσεις· το πρώτο όχι.

  **Δύο πραγματικοί ισχυρισμοί έπεσαν, και οι δύο κρέμονται από το ίδιο commit.** `spectral_flux_contract.rs:92,97`: `"Kick at 0ms not detected. Beats: {:?}"` / `"Kick at 1000ms not detected."`. `adversarial_drone.rs:121-123`: σχόλιο `// Force K=14 via music profile`, `assert_eq!(mean_h.len(), 14, "Music should load K=14")` — πήρε 8. Η πύλη, αυτούσια, `two_pass.rs:809,817`:
  ```rust
  let use_drums = is_music_profile && weighted_lean < 0.35 && sum_conf > 0.0;
  ...
  let nmfd_k = if use_drums { 14 } else { 8 };
  ```
  Τριών όρων, όχι ενός: το προφίλ είναι ένας στους τρεις — οι άλλοι δύο (`weighted_lean`, `sum_conf`) έρχονται από τον ανιχνευτή. Και το `UserSpatialProfile` (`sp314-dsp/src/spatial/user_profile.rs:4-10`) δεν έχει καν πεδίο K — πέντε τάσεις `f32` (`width_tendency`, `depth_tendency`, `center_strength`, `lfe_tendency`, `rear_decay`), τίποτα άλλο. Το δοκίμιο και η πύλη γράφτηκαν την ίδια μέρα, στο ίδιο commit (`0422d10`, 2026-08-18, επαληθευμένο `git log`) — άρα περνούσε τότε. Ο μόνος μετρημένος ενδιάμεσος στην αλυσίδα εισόδου της πύλης είναι το `ad627ca` (2026-09-17, επαληθευμένο `git log`): *«the onset detector takes its threshold from the signal, and a zipper goes away»* — Otsu πάνω στην κατανομή flux ανά παράθυρο αντί για σταθερά, με μετρημένο αποτέλεσμα στο ίδιο commit message: στα εννέα πραγματικά βιβλία τα ψευδή σύνορα έπεσαν 75→63 και 114→84 (ένα έκτο και ένα τέταρτο), μηδέν πραγματικό σύνορο χάθηκε στα δώδεκα με γνωστή απάντηση, ένα βγήκε byte-identical. ⚠ Δεν είναι οπισθοδρόμηση — είναι δύο φρουροί που φρουρούν συμπεριφορά που άλλαξε σκόπιμα. ⚠ Η αιτιώδης σύνδεση ανάμεσα στο `ad627ca` και τα δύο σπασμένα δοκίμια είναι συμπέρασμα από την αλυσίδα (`weighted_lean`/`sum_conf` τροφοδοτούνται από τον ανιχνευτή έναρξης) και τις ημερομηνίες, όχι μέτρηση με επαναφορά commit — γράφεται ως τέτοιο. ⚠ Το ίδιο το commit message του `ad627ca` δεν αναφέρει κανένα F-number· δεν αντιστοιχεί στο υπάρχον `FINDINGS.md` F-116 (τρία tests μνήμης, άσχετο θέμα) — καμία τρέχουσα καταχώρηση εδώ δεν τεκμηριώνει ονομαστικά αυτό το commit, μετρημένη απόκλιση, δεν διορθώνεται εδώ.
...
- **[F-135] Ο ανιχνευτής: τρία προσωρινά κατώφλια έξω από κάθε φρουρό, ένα νεκρό διπλό της κλιμάκωσης, και το «segmenter+classifier» του κλαδέματος οριστικά μη κλαδεύσιμο.** Component: `lineos/m1/lineos-corpus/src/scout.rs` · `lineos/m1/sp314-dsp/src/analysis/stem_escalation.rs` · `lineos/m0/m0-daemon/src/dsp/orchestrator/nmf_worker.rs` · `lineos/m1/sp314-orchestrator/src/trunk_pass.rs` · `lineos/m0/m0-daemon/src/{agents/executor.rs,domain/dsp_pipeline.rs,bin/features.rs}` · `scripts/threshold-lint.sh`.
  ...
  **ΔΙΟΡΘΩΣΗ 2026-09-21, διαβασμένο στο ίδιο το PRD:** το μετρημένο μέρος στέκει — το `run_trunk_pass` καλείται από τις δύο ζωντανές διαδρομές του `m0-daemon`, και δεν αφαιρείται χειρουργικά όσο ο διακομιστής υπάρχει. ⚠ Αλλά το συμπέρασμα «δεν κλαδεύεται» αφήνει να εννοηθεί ότι ταξιδεύει στο νέο μηχάνημα — δεν ταξιδεύει. Το §3 (μη-στόχοι) το ονομάζει ρητά με δύο ξεχωριστές γραμμές: «Content detection at drop» — «The voice/music classifier fell (F-108, F-110). Container probing is measurement and stays; the user chooses the profile.» — και «Mixing — balancing two signals against each other (ducking, same-stem speaker balancing, stem summing, separation-based processing)» — «K0.» Το §4 γράφει «Removed: the bedroom musician (D6).» Η λίστα «Wrong» του §11 τον ονομάζει ρητά: «the macro classifier as a voice/music gate (F-108, F-110)». Και το διάγραμμα του §6.0 δεν τον σχεδιάζει πουθενά: το κουτί του προφίλ λέει «the user chooses. no CONTENT detection. unknown profile = error», και το κουτί Α «no content detection: the probe is measurement» (όλα επαληθευμένα αυτούσια στο ζωντανό PRD). ⇒ Φεύγει με τον διακομιστή, όπως οι άλλες πέντε κατηγορίες του `docs/PRUNE-STATUS.md`. ⇒ Και τα τρία προσωρινά κατώφλιά του δεν χρειάζονται βαθμονόμηση — φρουρούνται πλέον (`6f7c8232`) ώστε να μην ξεχαστούν όσο ζουν, όχι ώστε να κλειδωθούν. ⚠ Και τα δύο ευρήματα (εδώ και στο F-136 παρακάτω) γράφτηκαν χωρίς να έχει διαβαστεί το §3 και το διάγραμμα του §6.0 στην πηγή· διαβασμένα, το κρίσιμο ήταν αυτό που ΔΕΝ υπάρχει μέσα στο διάγραμμα: κανένας ανιχνευτής, κανένας ταξινομητής, κανένα NMF, κανένας διαχωρισμός στελεχών.

- **[F-136] Το μισό του suite που κρατάει ντετερμινισμό και πιστοποιητικό έτρεξε για πρώτη φορά στη συνεδρία: δώδεκα κόκκινα, έντεκα δείγματα χαμένα σε /tmp, ένα πραγματικό.** Component: `scripts/run-ignored.sh` · `scripts/.ignored-baseline` · `lineos/m0/m0-daemon/tests/{glue_full_render.rs,w17_bed_sweep.rs,w17_mix_balance.rs,w6c_nmfd_cost.rs,w10_duck_dynamics.rs,w17_flacenc_bloat.rs,pass1_integration.rs,streaming_integration.rs,e2e_streaming_http_hybrid_smoke.rs,e2e_episode_streaming.rs}` · `lineos/m1/sp314-dsp/tests/{pass1_integration.rs,w17_voice_activation.rs}`.

  ΜΕΤΡΗΜΕΝΟ 21/09 στο μηχάνημα του χρήστη, `scripts/run-ignored.sh` και ξεχωριστά τρεξίματα ανά δοκίμιο — συνέχεια του F-134: εκείνο έτρεξε το πρώτο μισό του suite (`cargo test --workspace`), αυτό το δεύτερο μισό, τα `#[ignore]`d δοκίμια ντετερμινισμού/πιστοποιητικού, για πρώτη φορά σε όλη τη συνεδρία.

  **Το αποτέλεσμα: 43 αρχεία έτρεξαν, 15 εξαιρέθηκαν με γραμμένο λόγο, 12 κόκκινα έναντι παγωμένου 9.** Κανένα από τα δώδεκα δεν το προκάλεσε η δουλειά της συνεδρίας — επαληθεύτηκε ένα προς ένα με το ίδιο το μήνυμα αποτυχίας κάθε δοκιμίου.

  **Έντεκα στα δώδεκα είναι δείγματα που λείπουν, σε τέσσερις ρίζες με διαφορετική τύχη.** `/tmp/w9/podcast_realistic.wav` — τέσσερα δοκίμια (`glue_full_render`, `w17_bed_sweep`, `w17_mix_balance`, `w6c_nmfd_cost`): το μήνυμά τους λέει συνταγή μόνο περιγραφική στο `52e2a31` (επαληθεύτηκε ο commit), πηγές ακαταγράφητες (F-072), το ξαναχτίσιμο δίνει άλλο αρχείο. `/tmp/w9/podcast_loud_bed.wav` (`w10_duck_dynamics`) και `/tmp/w17_nonorm/*.raw` (`w17_flacenc_bloat`) — άγνωστος τρόπος ανακατασκευής, το δεύτερο χειροκίνητο (F-078). `flight_clips_stereo/` — δύο (`pass1_integration` σε δύο crates, `streaming_integration`), ίδια ρίζα με το F-134. `/tmp/w17_pure` — ένα (`w17_voice_activation`), και είναι το μόνο που ανακτάται: το ίδιο το μήνυμά του δίνει τη διαδρομή διάσωσης (`~/creator-os-attic/tmp-fixtures-2026-08/w17_pure/`, διασώθηκε 21/08), και τα αρχεία είναι εκεί. ⚠ Ένα ακόμα είναι το ίδιο πράγμα τυλιγμένο αλλιώς: `e2e_streaming_http_hybrid_smoke` αποτυγχάνει με «Final stage should be COMPLETED, left: ERROR», και το σφάλμα από κάτω είναι «standardized decode open failed: No such file or directory» — δείγμα, όχι λογική.

  **Και η ρίζα είναι χειρότερη από «λείπουν»: έξι δοκίμια ψάχνουν αρχεία στο `/tmp`**, που το λειτουργικό το αδειάζει. Δεν είναι ατυχία — είναι εγγυημένο να συμβεί. Και όταν συνέβη, τρία από τα τέσσερα μηνύματα λένε ρητά ότι δεν ξαναφτιάχνονται. ⚠ Τα μηνύματα είναι υποδειγματικά — λένε τι λείπει, γιατί, και πού να ψάξεις (επαληθεύτηκε αυτούσιο σε κάθε ένα παραπάνω). Το πρόβλημα δεν είναι η καταγραφή· είναι ότι η καταγραφή έγινε αφού χάθηκαν. Τρίτη φορά σε δύο μέρες το ίδιο σχήμα: το UD map, τα `flight_clips`, και τώρα το `/tmp` — χτισμένο και αόρατο στο δέντρο.

  **Ένα είναι πραγματικό, και είναι απαίτηση που δεν τηρείται.** `e2e_episode_streaming::music_pipeline_heap_is_scale_invariant` έτρεξε 185.73s και απέτυχε με μετρημένα νούμερα: `music_pipeline heap (spotify): 1m=176.29MB · 2m=232.58MB · diff=56.29MB` ⇒ «heap grew 56.29MB when duration doubled — music pipeline is NOT scale-invariant». Το `R8` του PRD λέει «Disk-streaming pipeline, bounded RSS» με τη σημείωση *Ready* (`docs/specs/PRD-v7.2-20260918.md:273`, επαληθευμένο)· το §6.0 «one pass, O(1) memory» (`:368`, επαληθευμένο). Το *Ready* είναι ψευδές για τη μουσική διαδρομή — το δοκίμιο το ξέρει από τότε που γράφτηκε και κανείς δεν το έτρεχε. Γράφεται ως το μόνο πραγματικό των δώδεκα, χωρίς προτεινόμενη αιτία — δεν μετρήθηκε.

  **Και μία ανοιχτή αδυναμία του φρουρού.** `scripts/.ignored-baseline` είναι δύο bytes — σκέτο `9\n` (επαληθεύτηκε `od -c`), χωρίς ονόματα, mtime 2026-09-18. Το +3 δεν μπορεί να αποδοθεί σε συγκεκριμένα δοκίμια· ξέρουμε μόνο ότι κανένα από τα δώδεκα δεν είναι δικό της συνεδρίας. ⚠ Ίδιο σχήμα με το F-132: νούμερο χωρίς ταυτότητα. Καταγράφεται, δεν διορθώνεται εδώ.

  **ΔΙΟΡΘΩΣΗ 2026-09-21, διαβασμένο στο ίδιο το PRD:** το δοκίμιο που έπεσε είναι το `music_pipeline_heap_is_scale_invariant`, και το ίδιο αρχείο έχει άλλα δύο του ίδιου είδους — `episode_render_heap_is_scale_invariant` και `full_pipeline_heap_is_scale_invariant` (και τα δύο επαληθεύτηκαν στο ζωντανό `e2e_episode_streaming.rs`) — που **πέρασαν** στο ίδιο τρέξιμο («0 passed; 1 failed; 4 filtered out»). ⇒ Η κοινή ρίζα είναι καθαρή· φουσκώνει μόνο η μουσική διαδρομή. ⚠ Και η μουσική διαδρομή είναι μη-στόχος, με τα τέσσερα τεκμήρια της διόρθωσης του F-135 παραπάνω (§3 δύο γραμμές, §4 «Removed: the bedroom musician (D6)», §11 «Wrong» ονομάζει ρητά τον ταξινομητή F-108/F-110, το διάγραμμα του §6.0 δεν τον σχεδιάζει πουθενά). ⇒ Άρα το «ένα πραγματικό στα δώδεκα» εδώ πάνω γίνεται **μηδέν πραγματικά για το προϊόν**: έντεκα δείγματα που λείπουν, και ένα δοκίμιο που φρουρεί κώδικα που δεν παραδίδεται. ⇒ Και το «bounded RSS, *Ready*» του R8 δεν διαψεύδεται από αυτό: τα δύο δοκίμια που μετράνε τις διαδρομές του προϊόντος πέρασαν. ⚠ Και το ίδιο το διάγραμμα γράφει το «one pass, O(1) memory» **μόνο μέσα στο κουτί Α**, τη μέτρηση — όχι στην αλυσίδα διόρθωσης και όχι σε διαδρομή που δεν σχεδιάζεται (επαληθευμένο, `docs/specs/PRD-v7.2-20260918.md:366-369`).

- **[F-137] Η μετακόμιση των δύο τελευταίων συναρτήσεων της μηχανής μετρήθηκε ξανά και σταμάτησε στο Βήμα 0 — δύο ανεξάρτητοι λόγοι, ο πρώτος σε βάθος δύο που το F-133 δεν είχε ελέγξει, ο δεύτερος στο ποιος αλλιώς καλεί τις δεκαεπτά τοπικές.** Component: `lineos/m0/m0-daemon/src/{agents/executor.rs,handlers/deliver.rs,handlers/pdf_gen.rs,handlers/certificate.rs,identity.rs,config.rs,domain/dsp_pipeline.rs,domain/nodes/certificate_node.rs,handlers/master.rs,handlers/export.rs,domain/nodes/decode_node.rs,agents/conductor.rs,agents/operator.rs,app_state.rs,blob_store.rs,spool.rs,handlers/timeline.rs,domain/nodes/{dsp_node.rs,scout_node.rs},dsp/{input_lufs.rs,lazy_reader.rs,wav_to_raw.rs,orchestrator/nmf_worker.rs}}` · `docs/specs/PRD-v7.2-20260918.md` §5.1, §6.1, Appendix B, R7 · `DECISIONS.md` §7 απόφαση 3.

  ΜΕΤΡΗΜΕΝΟ 2026-09-21, ίδιο δέντρο, μετά την τομή του παράπλευρου (`e83b226e` κλάδου εργασίας, canonical `bfeda1b`), ως άμεση συνέχεια/διόρθωση του F-133.

  **Τα νούμερα του F-133 ξαναμετρήθηκαν, ΚΑΙ διορθώνεται ένα δικό μου λάθος του προηγούμενου βήματος.** `execute_streaming_plan` (`agents/executor.rs:230-733`): **21 μοναδικές διαδρομές `crate::`, 37 συνολικές εμφανίσεις** — επαληθεύτηκε τρεις φορές με `grep -oE`, ταιριάζει ΑΚΡΙΒΩΣ με το αριθμό του F-133. ⚠ Στο προηγούμενο recon (στο ίδιο task, πριν από αυτή τη γραφή) είχα αναφέρει «20» — μέτρημα με το μάτι πάνω σε είκοσι μία γραμμές `uniq -c`, λάθος κατά ένα, δικό μου, όχι του δέντρου. Διορθώνεται εδώ, όχι εκεί. `run_deliver_core` (`handlers/deliver.rs:454-679`, μετά την τομή): **3, όχι 4** — ακριβώς επειδή το `find_sidecar`/`write_sidecar` έφυγαν στο `e83b226e`. **Σύνολο μοναδικών διαδρομών: 24, όχι 25 ΚΑΙ όχι 23** (το 23 ήταν το ίδιο λάθος μέτρημα παραπάνω, μεταφερμένο).

  **Ο διαχωρισμός επανεξαγωγή/τοπική επαναταξινομήθηκε από την αρχή, ανά πλήρη διαδρομή — όχι ανά τύπο.** Επανεξαγωγές (ο κώδικας ζει ήδη αλλού): `CorrectionRecord`/`NamedValue`/`StoredBlobV2` (`blob_store.rs:882,891` ← `lineos_types::certificate`), `ContentType` (`domain/content_type.rs:4` ← `aether_bridge`), `TARGET_SR` (`dsp/stream_core.rs:12` ← `lineos_types::analysis`), `DeliveryCheck::from_format` ΚΑΙ `DeliveryCheck::from_spacing` (`blob_store.rs:37` ← `lineos_types::certificate`, ΔΥΟ ξεχωριστές διαδρομές, όχι μία — και τα δύο σώματα επαληθεύτηκαν αυτούσια στο `lineos-types/src/certificate.rs:511,575`, ΟΧΙ στο `blob_store.rs`) — **7, όχι 11**. Γνήσιες τοπικές: `blob_store::{mastered_path, raw_dump_path, DeliveryVerdict::compose}` · `domain::dsp_pipeline::{compute_sha256_bytes, derive_seed}` · `domain::nodes::certificate_node::{StreamingCertData, run_streaming}` · `domain::nodes::dsp_node::build_intent_and_config` · `domain::nodes::scout_node::run` · `dsp::input_lufs::pass0_decode_to_dump` · `dsp::lazy_reader::read_scout_sample` · `dsp::orchestrator::nmf_worker::{dispatch_all_jobs, spawn}` · `dsp::wav_to_raw::wav_to_raw_measured` · `handlers::timeline::TimelineProfiler` · `spool::spool_dir` · `app_state::MasteringProgress` — **17, όχι 14**, σε δώδεκα αρχεία, από τα οποία **τέσσερα δεν είναι κάτω από `domain/` ή `dsp/`**: `blob_store.rs`, `spool.rs`, `handlers/timeline.rs`, `app_state.rs` ζουν στη ρίζα του `src/`. ⚠ Το 7+17=24 συμφωνεί με το νέο σύνολο· το 11+14=25 του F-133 δεν συμφωνούσε ούτε με το δικό του σύνολο των 25 στην εποχή του — μετρημένο εδώ ότι το `find_sidecar`/`write_sidecar` (τα δύο στοιχεία που το F-133 μέτρησε ως μέρος του `run_deliver_core`, πριν φύγουν στο `e83b226e`) ήταν και τα δύο γνήσιες τοπικές συναρτήσεις του `blob_store.rs` (`:461,593`, επαληθεύτηκε), όχι επανεξαγωγές· ο διαχωρισμός 11/14 δεν ανακατασκευάζεται από την ίδια τη λίστα του F-133.

  **Το πρώτο εμπόδιο είναι σε βάθος δύο, που το F-133 δεν είχε ελέγξει** (εκείνο σταμάτησε σε βάθος ενός επιπέδου, ρητά). `run_streaming` καλεί το ιδιωτικό `assemble_blob` (ίδιο αρχείο, `domain/nodes/certificate_node.rs:339`), και εκείνο καλεί **τέσσερις νέες διαδρομές σε τέσσερα αρχεία εκτός των δώδεκα**: `crate::identity::load_or_generate_default` (`identity.rs:97`) · `crate::handlers::certificate::{sign_certificate, generate_qr_base64}` (`handlers/certificate.rs:58,6`) · `crate::config::M0Config::from_env` (`config.rs:31`) · `crate::handlers::pdf_gen::generate_silent_certificate` (`handlers/pdf_gen.rs:10`). Οι τρεις πρώτες είναι καθαρές — επαληθεύτηκε ανάγνωση αυτούσια των σωμάτων τους, μηδέν axum/db. Το `handlers/pdf_gen.rs` **έχει** `use axum::http::{header, StatusCode}` στη γραμμή 328 και έναν πραγματικό axum route handler από κει και πέρα (σερβίρει το PDF σε θύρα) — αλλά **οι συναρτήσεις που καλούνται** (`generate_silent_certificate`/`_generate`, γραμμές 10-201) **είναι οι ίδιες καθαρές**· το axum ζει σε άλλη συνάρτηση, ίδιο αρχείο. Η προδιαγεγραμμένη μέθοδος μεταφοράς («git mv ολόκληρο αρχείο, μηδέν αλλαγή σε σώμα») δεν έχει ασφαλή εφαρμογή εδώ: ολόκληρο το αρχείο σέρνει axum σε crate μηχανής, και η χειρουργική διάσπαση είναι αλλαγή σε σώμα αρχείου που η ίδια η μέθοδος αποκλείει.

  **Και το εύρημα δεν είναι το axum καθαυτό — είναι τι κάνει σήμερα ο πυρήνας: υπογράφει και τυπώνει χαρτί μέσα του.** Τρεις από τις τέσσερις νέες διαδρομές είναι υπογραφή (`sign_certificate`, `load_or_generate_default`) και απόδοση (`generate_silent_certificate` → PDF, `generate_qr_base64` → εικόνα μέσα στο PDF). Η ΑΠΟΦΑΣΗ 3 του `DECISIONS.md` §7 (canonical `0697e5d`, κείμενο επαληθευμένο αυτούσιο): «Η δήλωση είναι τιμή. Αποθήκευση, απόδοση και επαλήθευση είναι προσαρμογείς έξω από τον πυρήνα» — ήδη σημειώνει ρητά «ο πυρήνας γράφει ήχο, όχι χαρτί — έχει δύο σημεία δίσκου και το ένα είναι το ίδιο το παραδοτέο (παραπομπή στο F-133 μέσα στο πρωτότυπο, χωρίς αγκύλες εδώ ώστε να μη συγκρουστεί με τη μορφή τίτλου του allocator-lint)». Το R7 του PRD (`:257-264`, επαληθευμένο αυτούσιο) απαριθμεί τις αποδόσεις: «**HTML** self-contained verifier… **PDF** deterministic archival; **ADM `axml`** sidecar». ⇒ Ίδιο σχήμα με το παράπλευρο (sidecar), ένα επίπεδο βαθύτερα και με τρία πράγματα (υπογραφή + PDF + QR) αντί για ένα.

  **Το δεύτερο εμπόδιο είναι μεγαλύτερο: δέκα από τις δεκαεπτά γνήσιες τοπικές έχουν άλλον καλούντα στο m0-daemon**, επαληθευμένο με `grep` ανά πλήρη διαδρομή, εξαιρώντας `executor.rs`/`deliver.rs`: `raw_dump_path` (`domain/nodes/decode_node.rs` ×3, `domain/dsp_pipeline.rs` ×4) · `DeliveryVerdict` (`handlers/export.rs` ×5) · `StreamingCertData`, `run_streaming`, `build_intent_and_config`, `scout_node::run` (×2), `read_scout_sample`, `TimelineProfiler` — όλα από `domain/dsp_pipeline.rs` (τη μη-streaming διαδρομή, `run_dsp_internal`) · `spool_dir` (`blob_store.rs` ×8, εσωτερικά) · `MasteringProgress` (`agents/conductor.rs`, `agents/operator.rs`, `handlers/master.rs` ×9, `domain/dsp_pipeline.rs`). **Μόνο επτά** έχουν μηδέν άλλον καλούντα: `mastered_path` · `compute_sha256_bytes` · `derive_seed` · `pass0_decode_to_dump` · `nmf_worker::{spawn, dispatch_all_jobs}` · `wav_to_raw_measured`. ⚠ Το PRD §6.1 (`:467`, κλειδωμένο 11/09, επαληθευμένο αυτούσιο) ονομάζει «the `deliver` crate» ως προορισμό και δηλώνει «*Measured: engine crates already have zero server dependencies; the two functions merely live in the wrong crate.*» — ΚΑΙ το Appendix B, λίστα «Ready and tested» (επαληθευμένο αυτούσιο), επαναλαμβάνει «engine crates free of server dependencies». Και τα δύο διαψεύδονται από αυτή τη μέτρηση: η αξιόλογη πλειοψηφία του όγκου (`run_streaming`/`StreamingCertData`, ο μεγαλύτερος καλούμενος) είναι πλεγμένη με `run_dsp_internal`, `handlers/master.rs`, `handlers/export.rs`, `decode_node.rs` — τρίτη διαδρομή που το §6.1 δεν ονομάζει. Το ίδιο Appendix B, λίστα «Exists, unwired or partial», λέει σωστά ότι οι δύο συναρτήσεις είναι ακόμα «plain functions inside `m0d`» — αυτό συμφωνεί με το δέντρο. Καμία κρίση εδώ για το αν το `run_dsp_internal` πρέπει να μείνει ή να φύγει· το §5.1 (`:203`, ΟΧΙ §11 όπως δόθηκε στην εντολή — επαληθεύτηκε ότι η φράση δεν υπάρχει στο §11· η επικεφαλίδα που προηγείται της γραμμής 203 είναι «### 5.1 Must-have (P0) — Phase 1 MVP») μέτρησε ήδη ότι η διεπαφή στέλνει τα πάντα στο `/master/streaming` από τις 17/07 (`706d237`) — καταγράφεται η διόρθωση παραπομπής, όχι νέα κρίση.

  **Τρία νούμερα καλούντων διορθώνονται.** `execute_streaming_plan`: δύο σε `src/` (`agents/batch.rs:221`, `agents/executor.rs:154`) — ταιριάζει· μηδέν δοκίμια· **21 αρχεία στο `research/`** (όχι 17). `run_deliver_core`: ένας σε `src/` (`handlers/deliver.rs`, ο ίδιος ο handler) — ταιριάζει· **τέσσερα δοκίμια, όχι πέντε** (`f077_margin_in_certificate.rs`, `deliver_acx.rs`, `external_acx_ffmpeg_agreement.rs`, `output_acx_delivered_certificate.rs`)· **δύο αρχεία στο `research/`** (όχι δεκαοκτώ). Δύο αναφορές που θα μετρούνταν λάθος ως κλήσεις είναι σχόλια: `certificate_node.rs:448`, `conformance/export.rs:445` — επαληθεύτηκε αυτούσιο, καμία τη δεν είναι κλήση. Σύνολο μοναδικών αρχείων `research/` (ένωση): **22, όχι 18**.

  **Μεθοδολογικό.** Το F-133 έλεγξε βάθος ένα και βρήκε είκοσι πέντε καθαρές διαδρομές· το βάθος ενός επιπέδου δεν έφτανε στο `assemble_blob`. Το βάθος δύο έβγαλε τέσσερα αρχεία εκτός λίστας και ένα από αυτά σέρνει axum μέσα από το ΙΔΙΟ αρχείο με καθαρές συναρτήσεις. Το ίδιο το PROTOCOL το προβλέπει: η λίστα των δεσμών είναι ελλιπής εξ ορισμού. Εδώ το έπιασε το Βήμα 0 πριν από οποιαδήποτε μεταγλώττιση — καμία αλλαγή κώδικα έγινε, καμία χρειάστηκε.

---

**NEXT FREE: F-138** — this line is the ONLY allocator. Taking a number =
