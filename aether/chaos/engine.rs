...
use crate::mapping::types::{
    ...
    COMP_ATTACK_DELTA_MIN_MS, COMP_RELEASE_DELTA_MAX_MS, COMP_RELEASE_DELTA_MIN_MS,
    EQ_GAIN_DELTA_MAX_DB, EQ_GAIN_DELTA_MIN_DB, SAT_DRIVE_DELTA_MAX, SAT_DRIVE_DELTA_MIN,
    STEREO_WIDTH_DELTA_MAX, STEREO_WIDTH_DELTA_MIN,
};
...
impl ChaosEngine {
    ...
    pub fn new(seed: u64) -> Self {
        let x0 = (seed as f32 / u64::MAX as f32).clamp(0.001_f32, 0.999_f32);
        let mut x = x0;
    ...
    pub fn next_delta(&mut self, chaos_intensity: f32, profile: &ChaosProfile) -> ChaosDelta {
        let ci = chaos_intensity.clamp(0.0_f32, 1.0_f32);
        if ci == 0.0 {
            return ChaosDelta::zero();
        }
    ...
    pub fn modulate(base: f32, depth: f32, chaos_val: f32, min: f32, max: f32) -> f32 {
    ...
    pub fn apply(base: &MicroDelta, delta: &ChaosDelta) -> MicroDelta {
        MicroDelta {
            eq: EqDelta {
                ...
                high_shelf_gain_db: (base.eq.high_shelf_gain_db + delta.air_shimmer_db)
            ...
            dynamics: DynamicsDelta {
                ...
                comp_attack_ms: (base.dynamics.comp_attack_ms + delta.comp_attack_mod)
                    .clamp(COMP_ATTACK_DELTA_MIN_MS, COMP_ATTACK_DELTA_MAX_MS),
                comp_release_ms: (base.dynamics.comp_release_ms + delta.comp_release_mod)
...
mod tests {
    ...
    fn chaos_apply_air_shimmer_gain_only() {
        ...
        assert!(
            (result.eq.high_shelf_gain_db - (base.eq.high_shelf_gain_db + 0.3)).abs() < 1e-4
    ...
    fn chaos_serializable() {
        let d = ChaosDelta {
            stereo_width_mod: 0.05,
            sat_drive_mod_db: 0.3,
            comp_release_mod: 5.0,
            comp_attack_mod: 1.0,
            air_shimmer_db: 0.2,
        };
