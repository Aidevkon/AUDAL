...
impl MacroMicroMapper {
    ...
    pub fn map(persona: &PersonaConfig,
        ...
        let w = apply_curve(macros.tone,
                            &persona.macros.tone.curve);
        let p = apply_curve(macros.dynamics,
                            &persona.macros.dynamics.curve);

        // ── EQ ────────────────────────────────────────────────
        let low_shelf_gain = (w * 3.0)
            .clamp(EQ_GAIN_DELTA_MIN_DB, EQ_GAIN_DELTA_MAX_DB);
        let low_shelf_freq = (w * (-200.0))
...
mod tests {
    ...
    fn mapping_deterministic() {
        ...
        let m = MacroControls { tone:0.5, dynamics:0.5 };
    ...
    fn mapping_zero_macros_near_zero_delta() {
        let p = default_persona();
        let m = MacroControls { tone:0.0, dynamics:0.0 };
    ...
    fn mapping_dynamics_tightens_compressor() {
        let p  = default_persona();
        let lo = MacroMicroMapper::map(&p, &MacroControls {
            tone:0.5, dynamics:0.1 });
        let hi = MacroMicroMapper::map(&p, &MacroControls {
            tone:0.5, dynamics:0.9 });
    ...
    fn mapping_stereo_width_always_zero() {
        ...
        for v in [0.0_f32, 0.5, 1.0] {
            let m = MacroControls { tone:v, dynamics:v };
    ...
    fn mapping_bounds_respected() {
        let p = default_persona();
        let m = MacroControls { tone:1.0, dynamics:1.0 };
        let d = MacroMicroMapper::map(&p, &m);
    ...
    fn tone_above_zero_produces_nonzero_sat_mix() {
        ...
        assert!(delta.sat.mix > 0.0,
            "tone=0.5 must produce sat_mix > 0.0, got {}", delta.sat.mix);
    }
}
