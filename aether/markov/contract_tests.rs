...
mod contract_tests {
    ...
    fn full_pipeline_deterministic() {
        ...
        let metrics = StemMetrics { rms_db: -20.0, crest_factor_db: 14.0, ..Default::default() };
    ...
    fn chaos_bypass_zero_cpu() {
        let metrics = StemMetrics { rms_db: -15.0, crest_factor_db: 8.0, ..Default::default() };
        let current = MarkovStateClassifier::classify_voice(&metrics);
        let predicted = MarkovStateClassifier::predict_next(current);
        let delta = PredictiveController::compute_voice_delta(current, predicted);
        
        let chaos_on  = ChaosLayer { bypass: false, seed: 42 };
        let chaos_off = ChaosLayer { bypass: true,  seed: 42 };
