...
use super::config::{PersonaConfig, MacroControls, PersonaOverride,
                    PERSONA_OVERRIDE_DELTA_MAX};
...
mod tests {
    ...
    fn personas_get_all_four() {
        let mgr = PersonaManager::load();
        for id in ["warm_analog","clean_punch","hybrid_hifi","cinematic_wide"] {
    ...
    fn personas_chaos_profiles_bounded() {
        ...
        for id in mgr.list() {
            ...
            for depth in [
                p.chaos.width_depth, p.chaos.drive_depth,
                p.chaos.release_depth, p.chaos.attack_depth,
                p.chaos.air_depth,
            ] {
                assert!(depth >= 0.0 && depth <= 1.0,
                    "chaos depth out of bounds in {}: {}", id, depth);
            }
        }
    }
