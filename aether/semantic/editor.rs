...
impl ZoneEditor {
    ...
    pub fn set_zone_gain(
        state: &mut ZoneEditorState,
        zone_id: &str,
        gain_db: f32,
    ...
    pub fn toggle_zone(state: &mut ZoneEditorState, zone_id: &str) -> Result<(), ZoneEditorError> {
        check_editable(state)?;
        let zone = find_zone_mut(state, zone_id)?;
    ...
    pub fn reset(state: &mut ZoneEditorState, persona: &PersonaConfig, features: &StemFeatures) {
...
fn compute_visuals(zones: &[SemanticZone], resolved: &ZoneAdjustments) -> Vec<ZoneVisual> {
    use std::collections::BTreeSet;

    let components = SemanticZoneResolver::find_components(zones);
    let colliding: BTreeSet<usize> = components
        .iter()
        .filter(|c| c.len() > 1)
    ...
    zones
        ...
        .map(|(i, zone)| {
            ...
            let resolved_band = resolved
                .bands
                .iter()
                .min_by(|a, b| {
                    (a.center_hz - zone.center_hz)
                        .abs()
...
mod tests {
    ...
    fn editor_collision_reflects_s007() {
        ...
        let active_count = s.zones.iter().filter(|z| z.active).count();
    ...
    fn zone_colors_defined_for_all_builtin() {
        for id in [
            ...
            assert_ne!(zone_color(id), "#95A5A6", "Missing color for zone: {}", id);
        }
    }
