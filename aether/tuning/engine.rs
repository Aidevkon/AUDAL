...
impl AteEngine {
    ...
    pub fn compute_deviation(reference: &StemFeatures, output: &StemFeatures) -> DeviationVector {
        ...
        let d_trans = ((reference.drums.crest_factor_db - output.drums.crest_factor_db) / 20.0_f32)
            .clamp(-1.0_f32, 1.0_f32);

        // Loudness: LUFS difference
        let d_loud = reference.mix.integrated_lufs - output.mix.integrated_lufs;
    ...
    pub fn deviation_to_override(dev: &DeviationVector) -> PersonaOverride {
        ...
        let tone_delta = (dev.delta_tilt_db * ATE_TILT_TO_WARMTH
            + dev.delta_body_db * ATE_BODY_TO_WARMTH)
            .clamp(-PERSONA_OVERRIDE_DELTA_MAX, PERSONA_OVERRIDE_DELTA_MAX);

        // Dynamics: transients
        let dynamics_delta = (dev.delta_transients * ATE_TRANS_TO_PUNCH)
            .clamp(-PERSONA_OVERRIDE_DELTA_MAX, PERSONA_OVERRIDE_DELTA_MAX);
    ...
    pub fn tune(reference: &StemFeatures, output: &StemFeatures) -> AteResult {
...
mod tests {
    ...
    fn ate_override_bounded() {
        let dev = DeviationVector {
            delta_tilt_db: 99.0,
            delta_body_db: -99.0,
            delta_transients: -99.0,
            delta_loudness_lufs: 99.0,
            converged: false,
        };
        let ov = AteEngine::deviation_to_override(&dev);
        assert!(ov.tone_delta.abs() <= PERSONA_OVERRIDE_DELTA_MAX + 1e-5);
        assert!(ov.dynamics_delta.abs() <= PERSONA_OVERRIDE_DELTA_MAX + 1e-5);
    }
    ...
    fn ate_tune_deterministic() {
        ...
        assert_eq!(
            r1.persona_override.tone_delta,
    ...
    fn ate_tone_positive_when_output_dark() {
        ...
        assert!(dev.delta_tilt_db > 0.0, "ref brighter → positive tilt");
        let ov = AteEngine::deviation_to_override(&dev);
        assert!(ov.tone_delta > 0.0, "positive tilt → positive tone_delta");
    }
    ...
    fn ate_result_serializable() {
        ...
        let _: AteResult = serde_json::from_str(&serde_json::to_string(&result).unwrap()).unwrap();
    }
}
