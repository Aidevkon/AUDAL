...
pub fn dispatch_hangar(mut state: Signal<HangarInterviewState>, event: HangarEvent) {
    web_sys::console::error_1(
        &format!(
            "[TRAP] HANGAR_DISPATCH: event={:?} state={:?}",
            event,
            *state.read()
        )
        .into(),
    ...
    web_sys::console::error_1(
        &format!("[TRAP] HANGAR_DISPATCH RESULT: next_state={:?}", next).into(),
    );
    state.set(next);
}

pub fn reduce_hangar(state: HangarInterviewState, event: HangarEvent) -> HangarInterviewState {
    use HangarEvent::*;
    use HangarInterviewState::*;
    match (state, event) {
        (AwaitingDrop, FilesDropped { count }) => Detection { track_count: count },

        (Detection { track_count }, AddMoreRequested) => AwaitingMore { track_count },

        (Detection { .. }, DetectionTimeout) | (AwaitingMore { .. }, DetectionTimeout) => {
            PlatformCard { selected: None }
        }

        (PlatformCard { .. }, PlatformChosen { platform }) => FlavourCard {
            platform,
            track_count: 1,
        },

        (FlavourCard { platform, .. }, FlavourChosen { flavour }) => Ignition { platform, flavour },
...
mod tests {
    ...
    fn drop_transitions_to_detection() {
        let r = reduce_hangar(AwaitingDrop, HangarEvent::FilesDropped { count: 1 });
