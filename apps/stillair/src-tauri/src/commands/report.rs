...
pub async fn export_pdf_report(
    ...
    eprintln!(
        "[report] BMR-128 PDF saved: blob_id={} path={}",
        blob.id, output_path
    );
...
pub async fn preview_pdf_report(
    ...
    // Write to temp file
    let tmp_path = std::env::temp_dir().join(format!("bmr128_{}.pdf", &blob_id[..8]));
    let tmp_str = tmp_path.to_string_lossy().to_string();

    generate_bmr128_pdf(&blob, &tmp_str)?;

    let pdf_bytes = tokio::fs::read(&tmp_path)
        .await
...
fn generate_bmr128_pdf(blob: &GoldenBlobJson, path: &str) -> Result<(), String> {
    let (doc, page1, layer1) =
        PdfDocument::new("BMR-128 Compliance Report", Mm(210.0), Mm(297.0), "Layer 1");
    ...
    let loudness_data: [(&str, String); 6] = [
        (
            "Integrated LUFS",
            format!("{:.1} LUFS", blob.loudness.integrated_lufs),
        ),
        (
            "True Peak",
            format!("{:.1} dBTP", blob.loudness.true_peak_dbtp),
        ),
        ("Loudness Range", format!("{:.1} LU", blob.loudness.lra)),
        (
            "Short Term LUFS",
            format!("{:.1} LUFS", blob.loudness.short_term_lufs),
        ),
        (
            "Momentary LUFS",
            format!("{:.1} LUFS", blob.loudness.momentary_lufs),
        ),
        (
            "EBU R128 Target",
            format!("{:.0} LUFS", blob.loudness.ebu_r128_target_lufs),
        ),
    ];
    ...
    let compliance_data: [(&str, bool); 7] = [
        ...
        (
            "Apple Music (-16 LUFS)",
            blob.loudness.apple_music_compliant,
        ),
        (
    ...
    let quality_data: [(&str, String); 5] = [
        (
            "Stereo Correlation",
            format!("{:.3}", blob.quality.stereo_correlation),
        ),
        (
            "Dynamic Range",
            format!("{:.1} dB", blob.quality.dynamic_range_db),
        ),
    ...
    doc.save(&mut std::io::BufWriter::new(
        std::fs::File::create(path).map_err(|e| format!("PDF create failed: {e}"))?,
    ))
