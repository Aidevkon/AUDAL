...
### [O-021] — conformance/src/spool.rs
- **Τι είναι:** Διαχείριση προσωρινών αρχείων (`spool_dir`,
  `init_spool_dir`) — υποδομή, όχι μηχανή. Μπήκε στο `conformance`
  στη μετακόμιση του `execute_streaming_plan`/`run_deliver_core`
  (F-137, 21/09) επειδή το `mastered_path` και το `raw_dump_path` το
  καλούν, όχι επειδή κρίθηκε ότι ανήκει εκεί. Ο μεταγλωττιστής το
  απαίτησε, όχι απόφαση.
- **Πού ζει:** `lineos/m1/conformance/src/spool.rs`
- **Βρέθηκε από:** F-137
- **Κατάσταση:** ΟΡΦΑΝΟ
- **Τύχη:** ΜΕΝΕΙ ΠΡΟΣΩΡΙΝΑ — φεύγει όταν αποφασιστεί πού ζει η
  υποδομή.

### [O-022] — conformance/src/mastering_progress.rs
- **Τι είναι:** `MasteringProgress`, ο διαύλος προόδου. Το §6.2 το
  βάζει στην **επικοινωνία** κελύφους-μηχανής, όχι στη μηχανή. Μπήκε
  στο `conformance` στην ίδια μετακόμιση (F-137, 21/09) επειδή η
  υπογραφή του `execute_streaming_plan` το χρειάζεται, όχι επειδή
  κρίθηκε ότι ανήκει εκεί. Ο μεταγλωττιστής το απαίτησε, όχι απόφαση.
- **Πού ζει:** `lineos/m1/conformance/src/mastering_progress.rs`
- **Βρέθηκε από:** F-137
- **Κατάσταση:** ΟΡΦΑΝΟ
- **Τύχη:** ΜΕΝΕΙ ΠΡΟΣΩΡΙΝΑ — φεύγει όταν υπάρξει κέλυφος· κέλυφος
  δεν υπάρχει ακόμα.
