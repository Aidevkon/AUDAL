...
1. **cockpit** — ΕΦΥΓΕ. Deleted with Tauri, `d0d9c70` (34,618 lines,
...
2. **JiniPanel and Ollama** — split fate, one name, two subjects.
   ...
   `d0d9c70`. The Ollama-calling backend logic is a separate thing and
...
4. **dead DSP nodes and their tests** — partial. Three of four ΕΦΥΓΑΝ,
   `28d1d09` (multiband, harmonic, lookahead_telemetry — 400 lines).
...
10. **Tauri** — ΕΦΥΓΕ, `d0d9c70` (with cockpit, same commit, same
    34,618-line deletion).
11. **SurrealDB and the six unused tables** — ΦΕΥΓΕΙ ΜΕ ΤΟΝ ΔΙΑΚΟΜΙΣΤΗ.
12. **Caddy** — ΕΦΥΓΕ, `3d71793` (267 lines — the field, its readers,
...
⚠ **Two hashes exist for every step in this session.** Each was sealed
first on the agent's branch, then taken onto `canonical` by hand — so a
`git cat-file` run inside the agent's tree fails on the canonical hash
and vice versa. The hashes above are the canonical ones, because that
is the branch this register is read from.
