...
*Searched 2026-09-08.* NUGEN AMB (tabbed queues, watch folders, logs; \~£700 core; enterprise scripting) — has compliance, lacks proof. DAW-resident meters (VisLM, Nuendo, Pro Tools) — **measurement is table stakes; we cannot sell it.** Auphonic Leveler — abandoned on modern macOS; position vacated. C2PA — export target, four-level disclosure model, browser verification without upload. Legal and healthcare — the competitor is "nothing".
...
- **Engine:** the `deliver` crate — `run_deliver_core` and `execute_streaming_plan` relocated out of `m0d`; measure, judge, correct, encode, sign. *Measured: engine crates already have zero server dependencies; the two functions merely live in the wrong crate.*
