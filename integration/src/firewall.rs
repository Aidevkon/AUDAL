...
mod tests {
    ...
    fn extreme_micro_delta() -> MicroDelta {
        ...
        MicroDelta {
            ...
            sat: SaturationDelta { drive: 99.0, mix: 99.0 },
    ...
    fn test_firewall_inputs() -> (PersonaConfig, MacroControls,
                                   MicroDelta, ZoneAdjustments,
                                   ChaosDelta) {
    ...
    fn firewall_build_deterministic() {
        let (persona, macros, micro, zones, chaos) =
            test_firewall_inputs();
    ...
    fn firewall_invalid_macro_fatal() {
        ...
        let result  = IntegrationFirewall::build(
            &persona, &macros,
            &MicroDelta::default(), &ZoneAdjustments::empty(),
            &ChaosDelta::zero(), 0, &mut log
        );
        assert!(matches!(result,
            Err(FirewallError::MacroOutOfRange { .. })));
    }
