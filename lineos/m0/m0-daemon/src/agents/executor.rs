    ...
    while let Some(intent) = rx.recv().await {
        match intent {
            ...
            Intent::RunStreaming { plan, response } => {
                ...
                let result = tokio::task::spawn_blocking(move || {
                    execute_streaming_plan(&plan, Some(progress_tx_clone))
                })
                .await;
...
pub fn execute_streaming_plan(
    plan: &super::operator::StreamingPlan,
    progress_tx: Option<tokio::sync::broadcast::Sender<crate::app_state::MasteringProgress>>,
) -> Result<(super::operator::StreamingOutput, crate::blob_store::StoredBlobV2), ExecutorError> {
    let audio_path = plan.audio_path.clone();
    ...
    let path_hash = crate::domain::dsp_pipeline::compute_sha256_bytes(audio_path.as_bytes());
    ...
    let (_, flagged_indices) = crate::dsp::orchestrator::nmf_worker::dispatch_all_jobs(
        &boundaries,
        &tx_job,
    );
