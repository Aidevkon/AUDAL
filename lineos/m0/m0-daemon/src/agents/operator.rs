...
// Moved to lineos-types 21/09 so the engine doesn't depend on the
// operator for its own signature (execute_streaming_plan uses all
// three).
pub use lineos_types::streaming::ExecutorError;
...
// operator for its own signature (execute_streaming_plan uses all
// three).
pub use lineos_types::streaming::{StreamingOutput, StreamingPlan};
