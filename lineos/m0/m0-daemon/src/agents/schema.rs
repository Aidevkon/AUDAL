...
impl ProjectState {
    ...
    fn validate_patch(&self, patch: &Value) -> Result<(), SchemaError> {
        let obj = patch.as_object().ok_or_else(||
            SchemaError::ValidationFailed("patch must be a JSON object".into())
        )?;
            ...
            match key.as_str() {
                ...
                "dsp.preset_id" => {
                    let v = val.as_str().ok_or_else(||
                        SchemaError::ValidationFailed(
                            "dsp.preset_id must be string".into()
                        )
                    )?;
                    const ALLOWED: &[&str] = &[
                        "spotify", "youtube", "apple_music",
                        "apple_podcast", "tidal", "broadcast",
                        "raw", "amazon",
                    ];
                    if !ALLOWED.contains(&v) {
                        return Err(SchemaError::ValidationFailed(
                            format!("dsp.preset_id '{v}' not allowed")
                        ));
                    }
                }
                "session.blob_id" => {
                ...
                "session.status" => {
                    let v = val.as_str().ok_or_else(||
                        SchemaError::ValidationFailed(
                            "session.status must be string".into()
                        )
                    )?;
                    const ALLOWED: &[&str] = &[
                    ...
                    if !ALLOWED.contains(&v) {
                        return Err(SchemaError::ValidationFailed(
                            format!("session.status '{v}' not allowed")
                        ));
                    }
                }
    ...
    while let Some(intent) = rx.recv().await {
        match intent {
            ...
            Intent::ValidateSchema { patch, response } => {
                let result = state.validate_patch(&patch)
                    .map(|()| {
...
mod tests {
    ...
    fn state() -> ProjectState { ProjectState::new() }
