use lineos_types::{LoudnessTarget, MasteringIntent, StereoBuffer};
...
fn assert_signal_integrity(signal: &[f32], node: &'static str) {
    ...
    assert!(
        !has_nan,
        "Signal integrity violation at '{}': NaN/Inf detected",
        node
    );
    assert!(
        !is_silent,
        "Signal integrity violation at '{}': audio silenced RMS={:.2e}",
        node, rms
    );
}
...
fn main() {
    ...
    let autotune_result =
        sp314_dsp::pipeline::autotune::autotune(pre_analysis.integrated_lufs, -14.0);
    ...
    println!("\n=== Output Metrics ===");
    println!(
        "  Integrated LUFS: {:.1} LUFS",
        output_analysis.integrated_lufs
    );
    println!(
        "  True Peak:       {:.1} dBTP",
        output_analysis.true_peak_dbtp
    );
    println!("  Peak:            {:.1} dBFS", out_peak_db);

    assert!(
        output_analysis.integrated_lufs > -20.0 && output_analysis.integrated_lufs < -8.0,
        "LUFS gate failed: {:.1} LUFS outside [-20, -8] range",
        output_analysis.integrated_lufs
    );
    assert!(
        output_analysis.true_peak_dbtp < -0.5,
        "True peak gate failed: {:.1} dBTP exceeds -0.5 ceiling",
        output_analysis.true_peak_dbtp
    );
    ...
    for i in 0..audio.left.len() {
        let l_smp = if audio.left[i] >= 0.0 {
            audio.left[i] * max_pos
        } else {
            audio.left[i] * max_neg
        };
        let r_smp = if audio.right[i] >= 0.0 {
            audio.right[i] * max_pos
        } else {
            audio.right[i] * max_neg
        };
        writer
            .write_sample(l_smp.clamp(-8388608.0, 8388607.0) as i32)
            .unwrap();
        writer
            .write_sample(r_smp.clamp(-8388608.0, 8388607.0) as i32)
            .unwrap();
    }
