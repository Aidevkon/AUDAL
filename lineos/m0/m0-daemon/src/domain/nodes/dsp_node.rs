...
use lineos_types::{MasteringIntent, LoudnessTarget, StemFeatures};
...
pub fn run(
    ...
    for s in chunk_left.iter_mut()  { *s *= gain_linear; }
    for s in chunk_right.iter_mut() { *s *= gain_linear; }
    ...
    let result = DspAdapter::master(
        &intent, left_slice, right_slice, sample_rate, Some(&dsp_config)
    ...
    Ok(DspOutput { lufs, true_peak, pre_analysis, dsp_config, proof_log, persona_config, aether_req })
}
