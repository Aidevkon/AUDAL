...
pub fn run(
    ...
    two_pass.process_chunks_with_params(mono, scout, ducking_gain, |stems_chunk| {
        ...
        let mh: Vec<f32> = stems_chunk.harmonics.iter().map(|s| s * mix.harmonics).collect();
        let ma: Vec<f32> = stems_chunk.ambience.iter().map(|s| s * mix.ambience).collect();

        h_voice.update(unsafe { std::slice::from_raw_parts(mv.as_ptr() as *const u8, mv.len() * 4) });
        h_drums.update(unsafe { std::slice::from_raw_parts(md.as_ptr() as *const u8, md.len() * 4) });
        h_bass.update(unsafe { std::slice::from_raw_parts(mb.as_ptr() as *const u8, mb.len() * 4) });
        h_harmonics.update(unsafe { std::slice::from_raw_parts(mh.as_ptr() as *const u8, mh.len() * 4) });
        h_ambience.update(unsafe { std::slice::from_raw_parts(ma.as_ptr() as *const u8, ma.len() * 4) });
        ...
        let stage = FiveDotOneStage::render_chunk(
            &clean_voice, &md, &mb, &mh, &ma, &scout.assignments,
        );
        let mut stage = stage;
    ...
    let original_rms = libm::sqrtf(
        original_left.iter().zip(original_right.iter())
            .map(|(l, r)| l * l + r * r)
            .sum::<f32>() / (original_left.len() * 2) as f32
    );
