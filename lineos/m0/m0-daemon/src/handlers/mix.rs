...
pub async fn post_commit(
    ...
    let hash = app.audio_repo.write().unwrap()
        .commit(new_state.clone(), &req.message);
    app.head_state_ptr.store(Arc::new(new_state));
    Json(MixResponse { ok: true, message: hash })
}
...
pub async fn post_checkout(
    ...
    match result {
        Ok(_) => {
            ...
            Json(MixResponse { ok: true, message: req.branch })
        }
...
pub async fn post_branch(
    ...
    match app.audio_repo.write().unwrap().create_branch(&req.name) {
        Ok(_)  => Json(MixResponse { ok: true,  message: req.name }),
        Err(e) => Json(MixResponse { ok: false, message: e }),
    }
}
...
pub async fn post_revert(State(app): State<AppState>) -> Json<MixResponse> {
    ...
    match result {
        Ok(_) => {
            let state = app.audio_repo.read().unwrap().head_state();
            app.head_state_ptr.store(state);
            Json(MixResponse { ok: true, message: "Reverted".into() })
        }
        Err(e) => Json(MixResponse { ok: false, message: e }),
    }
}
