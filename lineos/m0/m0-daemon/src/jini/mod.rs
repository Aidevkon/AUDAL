...
fn build_name_prompt(
    filenames: &[String],
    preset_id: &str,
    dsp_hint:  Option<&str>,
) -> String {
    let names_str = filenames.iter().take(10)
...
fn parse_name_suggestions(
    ...
    let names: Vec<String> = parsed.names.into_iter()
        .take(3).filter(|n| !n.is_empty()).collect();
    if names.is_empty() { return Err("No valid names".into()); }
    Ok(names)
}
