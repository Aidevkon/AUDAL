...
async fn test_agent_pipeline_executes_mastering() {
    ...
    let audit = Arc::new(
        m0d::audit::AuditLog::open(audit_dir)
    ...
    operator.dispatch(intent).await
    ...
    match result {
        Ok(output) => {
            ...
            assert!(!output.blob_id.is_empty(),
                "blob_id must not be empty");
...
async fn test_schema_agent_validates_and_queries() {
    ...
    let (tx, rx) = oneshot::channel();
    operator.dispatch(m0d::agents::operator::Intent::ValidateSchema {
        patch:    serde_json::json!({ "dsp.target_lufs": -16.0 }),
        response: tx,
    }).await.ok();

    let result = tokio::time::timeout(
        tokio::time::Duration::from_secs(5), rx
    ).await.expect("Timeout").expect("Channel dropped");
    assert!(result.is_ok(), "Valid patch must be accepted");

    // Test 2: query returns updated value
    let (tx2, rx2) = oneshot::channel();
    operator.dispatch(m0d::agents::operator::Intent::QuerySchema {
        path:     "dsp.target_lufs".into(),
        response: tx2,
    }).await.ok();

    let value = tokio::time::timeout(
        tokio::time::Duration::from_secs(5), rx2
    ).await.expect("Timeout").expect("Channel dropped");
    assert_eq!(value.as_f64().unwrap(), -16.0,
    ...
    let result3 = tokio::time::timeout(
        tokio::time::Duration::from_secs(5), rx3
    ).await.expect("Timeout").expect("Channel dropped");
