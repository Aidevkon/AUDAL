...
async fn e2e_corpus_integration_writes_model_to_disk() {
    ...
    assert!(std::path::Path::new(wav_path_rel).exists(), "Fixture missing");
    let wav_path = std::fs::canonicalize(wav_path_rel).unwrap().to_string_lossy().to_string();

    // Change to temp dir so corpus JSON + model JSON drop there
    let temp_dir = std::env::temp_dir()
        .join(format!("lineos_e2e_{}", uuid::Uuid::new_v4()));
    ...
    let has_corpus = files.iter().any(|f|
        f.starts_with("session_") && f.ends_with(".corpus.json"));
    assert!(has_corpus,
        "No session_*.corpus.json found. Files: {:?}", files);

    // Check UserMarkovModel was written
    let model_file = temp_dir.join("user_model_e2e_test_proj.json");
    assert!(model_file.exists(),
        "No user_model_e2e_test_proj.json found. Files: {:?}", files);
    ...
    assert!(model["presets"]["e2e_preset"].is_object(),
