...
fn maestro_calibration_broadband_signals() {
    ...
    let mono_a: Vec<f32> = kick.iter().zip(bass.iter())
        .map(|(k, b)| (k + b) * 0.5).collect();

    // Track B: hi-hat + bass = no collision
    let hhat = white_noise(dur, sr, 42);
    let mono_b: Vec<f32> = hhat.iter().zip(bass.iter())
        .map(|(h, b)| (h + b) * 0.5).collect();
    ...
    println!("  bass:  [{:.3}, {:.3}, {:.3}]",
        scout_a.stem_mfccs.bass[0],
        scout_a.stem_mfccs.bass[1],
        scout_a.stem_mfccs.bass[2]);
    println!("  drums: [{:.3}, {:.3}, {:.3}]",
        scout_a.stem_mfccs.drums[0],
        scout_a.stem_mfccs.drums[1],
        scout_a.stem_mfccs.drums[2]);

    println!("Scout B stem MFCCs (first 3 coeffs):");
    println!("  bass:  [{:.3}, {:.3}, {:.3}]",
        scout_b.stem_mfccs.bass[0],
        scout_b.stem_mfccs.bass[1],
        scout_b.stem_mfccs.bass[2]);
    println!("  drums: [{:.3}, {:.3}, {:.3}]",
        scout_b.stem_mfccs.drums[0],
        scout_b.stem_mfccs.drums[1],
        scout_b.stem_mfccs.drums[2]);
    ...
    println!("Track A (kick+bass): distance={:.3}, ducking={:.3}", dist_a, params_a.ducking_gain);
    ...
    println!("  dist_a={:.3} vs threshold 5.0:  {}",
        dist_a, if dist_a < 5.0 { "AGGRESSIVE (< 5.0)" }
                else if dist_a < 15.0 { "DEFAULT (5-15)" }
                else { "SUBTLE (> 15.0)" });
    println!("  dist_b={:.3} vs threshold 15.0: {}",
        dist_b, if dist_b < 5.0 { "AGGRESSIVE (< 5.0)" }
                else if dist_b < 15.0 { "DEFAULT (5-15)" }
                else { "SUBTLE (> 15.0)" });
}
