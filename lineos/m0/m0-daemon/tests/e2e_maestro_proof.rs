...
fn maestro_more_aggressive_ducking_for_bass_heavy_track() {
    ...
    let mono_a: Vec<f32> = bass_a.iter().zip(drums_a.iter())
        .map(|(b, d)| (b + d) * 0.5).collect();
    ...
    let mono_b: Vec<f32> = bass_b.iter().zip(hhat_b.iter())
        .map(|(b, h)| (b + h) * 0.5).collect();
    ...
    println!("Track A (50Hz + 60Hz): distance={:.2}, ducking_gain={:.3}",
        dist_a, params_a.ducking_gain);
    println!("Track B (50Hz + 8kHz): distance={:.2}, ducking_gain={:.3}",
