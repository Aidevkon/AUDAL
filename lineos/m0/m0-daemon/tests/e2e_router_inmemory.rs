...
async fn test_get_flavours_router_wiring() {
    ...
    assert!(
        json.get("active").is_some(),
        "Missing active field in response"
    );
    assert!(
        json.get("flavours").is_some(),
        "Missing flavours field in response"
    );

    let flavours_array = json.get("flavours").unwrap().as_array().unwrap();
    assert!(
        !flavours_array.is_empty(),
        "Flavours array should not be empty"
    );
}
