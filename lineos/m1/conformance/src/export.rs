...
//!
//! [2026-09-20] The signal-side writers moved here from m0-daemon's
//! handlers/export.rs (F-126): none of them touch axum, the blob_store
//! storage/signing primitives, or the database. What stayed behind —
//! export_audio (the axum handler), export_blob, export_mp3_routed,
//! write_deliverable_cert, delivery_checks_verdict (needs DeliveryVerdict,
//! which was never among the five certificate types moved to lineos-types
//! and still lives in m0-daemon's blob_store.rs) — is documented in
//! m0-daemon's own handlers/export.rs.
//!
...
use lineos_types::certificate::{DeliveryCheck, StoredBlobV2, StoredLoudness, StoredQuality};
...
pub fn export_wav(blob: &StoredBlobV2, path: &Path) -> Result<(), String> {
    let audio_bytes = std::fs::read(blob.core.audio_path.path())
        .map_err(|e| format!("Failed to read audio from disk: {e}"))?;
    if audio_bytes.is_empty() {
...
pub fn export_adm_bwf(blob: &StoredBlobV2, path: &Path) -> Result<(), String> {
...
pub fn export_opus(_blob: &StoredBlobV2, _path: &Path) -> Result<(), String> {
...
pub fn export_aiff(blob: &StoredBlobV2, path: &Path) -> Result<(), String> {
    let audio_bytes = std::fs::read(blob.core.audio_path.path())
        .map_err(|e| format!("Failed to read audio from disk: {e}"))?;
    if audio_bytes.is_empty() {
...
pub fn export_mp3(blob: &StoredBlobV2, path: &Path) -> Result<(), String> {
    use lame_sys::{
...
pub struct ExportSidecar<'a> {
    ...
    /// ό,τι υπάρχει.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub delivery_checks: Option<&'a [DeliveryCheck]>,
}
...
pub fn write_unsigned_export_sidecar(
    ...
    format: &str,
    audio_path: &Path,
    delivery_checks: Option<&[DeliveryCheck]>,
) -> Result<(), String> {
