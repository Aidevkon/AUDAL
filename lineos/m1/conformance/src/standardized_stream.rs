...
use crate::signal_health::{DeadAirSummary, SignalHealthMonitor};
use crate::stream_core::*;

pub use crate::stream_core::TARGET_SR;

// Same guards as decode_smart. Moved here 21/09 with the engine —
// m0-daemon's config.rs re-exports this so its nine unrelated callers
// don't change (same pattern as MAX_FILE_BYTES/decode.rs).
...
impl StandardizedAudioStream {
    pub fn open(path: &std::path::Path) -> Result<Self, String> {
        ...
        let reader = crate::lazy_reader::LazyAudioReader::open(path)
    ...
    pub fn tier1_verdict(
        &self,
        timing: crate::signal_health::VerdictTiming,
    ) -> Result<(), String> {
...
mod tests {
    ...
    fn batch_hashes(path: &str) -> (String, String) {
        let payload = crate::decode::decode_smart(path).expect("decode_smart");
