...
fn classify_for_stem(stem_type: &str, rms_db: f32, td: f32) -> &'static str {
    match stem_type {
        ...
        "drums" => {
            if rms_db < -50.0 { "quiet" }
            else if td > 0.30 { "transient" }
            else if rms_db > -20.0 { "sustain" }
            else { "decay" }
        }
        "bass" => {
            if rms_db < -50.0 { "silent" }
            else if td > 0.20 { "punchy" }
            else if rms_db > -25.0 { "sustained" }
            else { "rumble" }
        }
        "harmonics" => {
            if rms_db < -50.0 { "silent" }
            else if rms_db > -20.0 { "dense" }
            else { "sparse" }
        }
        _ => { // ambience
            if rms_db < -50.0 { "dry" }
            else if rms_db > -25.0 { "lush" }
            else { "subtle" }
        }
    }
}
...
pub fn build_timeline(
    ...
    let stem_pairs: &[(&str, &[f32], f32)] = &[
        ...
        ("harmonics", harmonics_audio, features.harmonics.transient_density),
    ...
    let stem_timelines: Vec<StemTimeline> = stem_pairs.iter().map(|(name, signal, td)| {
        ...
        let base_risk = RiskFlags {
            artifact_risk:  0.0,
            sibilance_risk: if pre_analysis.zone_flags.zone_cymbal_harsh { 0.7 } else { 0.0 },
            phase_issue:    if pre_analysis.zone_flags.zone_phase_issue { 0.8 } else { 0.0 },
