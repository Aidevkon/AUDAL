...
use crate::model::{EmissionHistogram, TransitionMatrix};
...
impl StemMarkovModel {
    ...
    pub fn train(&mut self, states: &[String], features: &[StateFeatures]) {
...
pub fn predict_state(
    ...
    let mut scores: Vec<(String, f32)> = states
        .iter()
        .map(|state| {
            let p_emission = model
                .emissions
                .get(state)
...
mod tests {
    ...
    fn trained_model() -> StemMarkovModel {
        ...
        let features: Vec<StateFeatures> = states
            .iter()
    ...
    fn scores_ordered_descending() {
        ...
        for i in 1..pred.scores.len() {
            assert!(
                pred.scores[i - 1].1 >= pred.scores[i].1,
                "Scores not sorted: {} < {}",
                pred.scores[i - 1].1,
                pred.scores[i].1
            );
