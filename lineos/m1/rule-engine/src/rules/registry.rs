...
use super::{dynamics, loudness, peak, stereo};
...
pub static RULES: &[Rule] = &[
    Rule {
        id: "lufs_compliance",
        check: loudness::lufs_compliance,
        tags: &["loudness"],
    },
    Rule {
        id: "true_peak_exceeded",
        check: peak::true_peak_exceeded,
        tags: &["peak", "critical"],
    },
    Rule {
        id: "dynamic_range_low",
        check: dynamics::dynamic_range_low,
        tags: &["dynamics"],
    },
    Rule {
        id: "lra_too_high",
        check: dynamics::lra_too_high,
        tags: &["dynamics"],
    },
    Rule {
        id: "stereo_correlation_weak",
        check: stereo::stereo_correlation_weak,
        tags: &["stereo"],
    },
    Rule {
        id: "dc_offset_detected",
        check: stereo::dc_offset_detected,
        tags: &["quality"],
    },
];
...
mod tests {
    ...
    fn test_rules_count() {
        assert_eq!(
            RULES.len(),
            6,
            "Expected exactly 6 core rules (v1.1 — lufs_compliance replaces 3)"
        );
    }
    ...
    fn test_no_old_loudness_rule_ids() {
        ...
        assert!(
            !ids.contains(&"lufs_too_high"),
            "lufs_too_high removed in v1.1"
        );
        assert!(
            !ids.contains(&"lufs_too_low"),
            "lufs_too_low removed in v1.1"
        );
        assert!(
            !ids.contains(&"lufs_apple_too_high"),
            "lufs_apple_too_high removed in v1.1"
        );
    }
}
