...
impl AlbumConductor {
    pub fn new(ear_model: EarFatigueModel, morph: MorphCurve) -> Self {
        Self { ear_model, morph, anchor_lufs: -14.0 }
    }
    ...
    pub fn plan_album(
        ...
        if analyses.is_empty() { return vec![]; }
        ...
        for (i, analysis) in analyses.iter().enumerate() {
            ...
            let target_lufs = Self::relative_lufs_target(
                anchor_lufs,
            ...
            let morph_frames = if i > 0 {
                self.morph.generate_frames(&prev_state, &opening_state, morph_frames_n)
            } else {
                vec![]
            };

            // Distance from PhantomMaster
            let phantom_distance = phantom.as_ref()
...
mod tests {
    ...
    fn relative_lufs_preserves_offset() {
        ...
        assert!((target - (-20.0)).abs() < 0.001,
            "6 LU gap should be preserved: expected -20.0, got {}", target);
    }
    ...
    fn anchor_track_hits_export_target() {
        ...
        assert!((target - (-14.0)).abs() < 0.001,
            "Anchor should hit export_target: expected -14.0, got {}", target);
    }
    ...
    fn aggressive_track_triggers_fatigue_on_next() {
        ...
        assert!(plans[1].opening_state.ducking_depth < base.ducking_depth,
