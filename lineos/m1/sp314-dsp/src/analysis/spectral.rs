...
pub fn spectral_centroid_hz(signal: &[f32], sample_rate: u32) -> f32 {
    ...
    if valid_frames == 0 { 1000.0 }
    else { total_centroid / valid_frames as f32 }
}
...
pub fn spectral_flatness(signal: &[f32]) -> f32 {
    if signal.is_empty() { return 0.5; }

    let mut engine = StftEngine::new();
    let (frames, n_frames) = engine.forward(signal);
    if n_frames == 0 { return 0.5; }
    ...
    if valid_frames == 0 { 0.5 }
    else { (total_flatness / valid_frames as f32).clamp(0.0, 1.0) }
}
