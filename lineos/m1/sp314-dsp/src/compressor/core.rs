...
impl CompressorBand {
    ...
    pub fn process(&mut self, x: f32) -> f32 {
        ...
        let gr_low_db = compute_gain_reduction(
            env_low_db,
            self.config.threshold_db,
            self.config.ratio,
            self.config.knee_db,
        );
        let gr_high_db = compute_gain_reduction(
            env_high_db,
            self.config.threshold_db,
            self.config.ratio,
            self.config.knee_db,
...
impl MultibandCompressor3 {
    ...
    pub fn process(&mut self, x: f32) -> f32 {
        ...
        let gr_low = libm::powf(
            10.0_f32,
            compute_gain_reduction(
                env_low_db,
                self.cfg_low.threshold_db,
                self.cfg_low.ratio,
                self.cfg_low.knee_db,
            ) / 20.0_f32,
        );

        let gr_mid = libm::powf(
            10.0_f32,
            compute_gain_reduction(
                env_mid_db,
                self.cfg_mid.threshold_db,
                self.cfg_mid.ratio,
                self.cfg_mid.knee_db,
            ) / 20.0_f32,
        );

        let gr_high = libm::powf(
            10.0_f32,
            compute_gain_reduction(
                env_high_db,
                self.cfg_high.threshold_db,
                self.cfg_high.ratio,
                self.cfg_high.knee_db,
            ) / 20.0_f32,
        );
