...
struct Lr4Splitter {
    lp_1: Biquad, lp_2: Biquad,
    hp_1: Biquad, hp_2: Biquad,
}
...
mod tests {
    ...
    fn measure_rms(signal: &[f32]) -> f32 {
        let mut sum_sq = 0.0;
        for &s in signal { sum_sq += s * s; }
        (sum_sq / signal.len() as f32).sqrt()
    }
    
    fn rms_to_db(rms: f32) -> f32 {
        if rms < 1e-10 { -200.0 } else { 20.0 * libm::log10f(rms) }
    }
    ...
    fn test_gate1_flat_sum() {
        ...
        for w in 0..num_windows {
            ...
            for i in 0..fft_size {
                // Hann window
                let win = 0.5 * (1.0 - libm::cosf(2.0 * core::f32::consts::PI * i as f32 / (fft_size as f32 - 1.0)));
    ...
    fn test_gate2_band_isolation() {
        ...
        assert!(db_10k_high - db_10k_mid > 20.0, "10kHz mid isolation failed");
        assert!(db_10k_high - db_10k_low > 20.0, "10kHz low isolation failed");
    ...
    fn test_gate3_remove_midrange() {
        ...
        assert!(rms_to_db(rms_1k_in) - rms_to_db(rms_1k_out) > 20.0, "1kHz not attenuated enough");
        ...
        assert!((rms_to_db(rms_50_in) - rms_to_db(rms_50_out)).abs() < 1.0, "50Hz dropped more than 1dB");
        ...
        println!("10kHz: in={} out={} diff={}", rms_to_db(rms_10k_in), rms_to_db(rms_10k_out), diff);
    ...
    fn test_gate4_lengths_and_no_nan() {
        let sr = 48000.0;
        let mut xover = Crossover3::new(sr, 200.0, 4000.0);
        let input: Vec<f32> = (0..48000).map(|i| if i % 2 == 0 { 1.0 } else { -1.0 }).collect();
