...
use lineos_types::{
    BehaviourVector, DynamicsBehaviour, FlavourId, JiniAction, JiniPersonaId, JiniSuggestion,
    LoudnessBehaviour, MacroHandle, QualityBehaviour, SpectralBehaviour, StereoBehaviour,
};
...
fn derive_action(b: &BehaviourVector) -> JiniAction {
    ...
    match &b.quality {
        ...
        QualityBehaviour::Silence => return JiniAction::SuggestNothing,
...
fn render_narrative(
    ...
    match persona {
        JiniPersonaId::Beginner => render_beginner(action, behaviour),
        JiniPersonaId::Intermediate => render_intermediate(action, behaviour),
        JiniPersonaId::Pro => render_pro(action, behaviour),
    }
}
...
fn render_beginner(action: &JiniAction, b: &BehaviourVector) -> String {
    match action {
        JiniAction::SuggestFlavourSwitch { to, .. } => match to {
            FlavourId::Clean => {
                "Your track sounds a bit heavy — Clean mode will open it up.".to_string()
            }
            FlavourId::Warm => {
                "Your track sounds a bit harsh — Warm mode will smooth it out.".to_string()
            }
        ...
        JiniAction::SuggestMacroChange { handle, delta, .. } => match handle {
            ...
            MacroHandle::Width => "Your track sounds quite narrow — let's open it up.".to_string(),
...
fn render_intermediate(action: &JiniAction, _b: &BehaviourVector) -> String {
    match action {
        ...
        JiniAction::SuggestNothing => {
            "Signal analysis complete — no adjustments needed.".to_string()
        }
    }
}
...
fn render_pro(action: &JiniAction, _b: &BehaviourVector) -> String {
    match action {
        JiniAction::SuggestFlavourSwitch { to, reason } => format!("{} → {:?}.", reason, to),
        JiniAction::SuggestMacroChange {
            handle,
            delta,
            reason,
        } => format!("{} {:?} {:+.2}.", reason, handle, delta),
        JiniAction::SuggestNothing => "No action required.".to_string(),
    }
}
