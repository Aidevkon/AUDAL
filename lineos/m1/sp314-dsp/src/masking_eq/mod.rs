...
impl MaskingAwareEQ {
    pub fn new(config: MaskingEQConfig, sample_rate: u32) -> Result<Self, ConfigError> {
        ...
        for (i, item) in hann_window.iter_mut().enumerate() {
            *item = 0.5
                * (1.0
                    - libm::cosf(2.0 * core::f32::consts::PI * i as f32 / (FFT_SIZE as f32 - 1.0)));
        }
        ...
        for b in 0..EQ_BANDS {
            let w0 =
                2.0_f64 * core::f64::consts::PI * (BAND_CENTER_HZ[b] as f64) / (sample_rate as f64);
            cos_w0[b] = libm::cos(w0);
            alpha[b] = libm::sin(w0) / (2.0_f64 * (BAND_Q as f64));
        }
    ...
    pub fn process_block(&mut self, left: &mut [f32], right: &mut [f32]) {
        ...
        for i in 0..left.len() {
            ...
            for b in 0..EQ_BANDS {
                ...
                self.current_coeffs[b] =
                    biquad::rbj_bell_fast(self.cos_w0[b], self.alpha[b], self.current_gain_db[b]);
            }

            let mut y_l = left[i];
            for b in 0..EQ_BANDS {
                y_l = biquad::process_tdf2(
                    y_l,
                    &self.current_coeffs[b],
                    &mut self.filter_states_l[b],
                );
            }

            let mut y_r = right[i];
            for b in 0..EQ_BANDS {
                y_r = biquad::process_tdf2(
                    y_r,
                    &self.current_coeffs[b],
                    &mut self.filter_states_r[b],
                );
            }
    ...
    fn run_analysis(&mut self) {
        ...
        psychoacoustic::bark::calculate_mask_per_bin(
            &spectrum_db,
            FFT_SIZE,
            self.sample_rate,
            &mut mask_buf,
        );
        psychoacoustic::iso226::apply_equal_loudness_weighting(
            &mut spectrum_db,
            self.sample_rate,
            FFT_SIZE,
            self.config.target_phon,
        );
        ...
        for (b, &center_hz) in BAND_CENTER_HZ.iter().enumerate() {
            ...
            self.gain_step_db[b] =
                (self.target_gain_db[b] - self.current_gain_db[b]) / HOP_SIZE as f64;
        }
    }
