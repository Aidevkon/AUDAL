...
pub const BARK_BOUNDARIES_HZ: [f32; 26] = [
    ...
    1270.0, 1480.0, 1720.0, 2000.0, 2320.0, 2700.0, 3150.0, 3700.0, 4400.0, 5300.0,
    6400.0, 7700.0, 9500.0, 12000.0, 15500.0, 20000.0
];
...
pub fn calculate_mask_per_band(spectrum_db: &[f32], fft_size: usize, sample_rate: u32, output: &mut [f32; MPEG1_BARK_BANDS]) {
    let num_bins = (fft_size / 2) + 1;
    debug_assert_eq!(spectrum_db.len(), num_bins, "spectrum_db length must be (fft_size/2)+1, got {}", spectrum_db.len());
...
pub fn calculate_mask_per_bin(spectrum_db: &[f32], fft_size: usize, sample_rate: u32, output: &mut [f32]) {
    let num_bins = (fft_size / 2) + 1;
    debug_assert_eq!(spectrum_db.len(), num_bins, "spectrum_db length must be (fft_size/2)+1, got {}", spectrum_db.len());
    debug_assert_eq!(output.len(), num_bins, "output length must match spectrum_db");
