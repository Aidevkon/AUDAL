...
pub const ISO226_80PHON_CORRECTION_DB: [f32; 29] = [
    25.8, 19.8, 13.9, 8.2, 2.5, -3.5, -9.3, -14.1, -18.1, -22.3, -25.8, -29.1, -31.6, -33.2, -33.8,
    -33.5, -32.2, -30.0, -27.5, -25.7, -27.0, -29.5, -32.3, -34.6, -35.1, -33.5, -30.4, -25.9,
    -20.5,
];

pub const ISO226_90PHON_CORRECTION_DB: [f32; 29] = [
    16.0, 10.0, 4.3, -1.3, -6.8, -12.6, -18.3, -23.1, -27.0, -31.1, -34.5, -37.7, -40.1, -41.7,
    -42.2, -41.9, -40.6, -38.4, -35.9, -34.1, -35.4, -37.8, -40.6, -42.9, -43.4, -41.8, -38.7,
    -34.2, -28.8,
];
...
pub fn interpolate_iso226_correction(freq_hz: f32, phon_level: f32) -> f32 {
    let freq_hz = libm::fmaxf(
        ISO226_FREQS_HZ[0],
        libm::fminf(ISO226_FREQS_HZ[28], freq_hz),
    );
...
pub fn apply_equal_loudness_weighting(
    spectrum_db: &mut [f32],
    sample_rate: u32,
    fft_size: usize,
    target_phon: f32,
    ...
    debug_assert_eq!(
        spectrum_db.len(),
        num_bins,
        "spectrum_db length must be (fft_size/2)+1"
    );
