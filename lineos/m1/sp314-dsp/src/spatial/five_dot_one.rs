...
impl SpatialFirewall {
    pub fn apply(&self, stage: &mut FiveDotOneStage) {
        ...
        for i in 0..len {
            front_energy_sq += stage.l[i] * stage.l[i] + stage.r[i] * stage.r[i] + stage.c[i] * stage.c[i];
...
impl FiveDotOneStage {
    ...
    pub fn render(
        ...
        for i in 0..len {
            ...
            c[i]  += v * assignments.voice.center_weight + d * assignments.drums.center_weight + b * assignments.bass.center_weight + h * assignments.harmonics.center_weight + a * assignments.ambience.center_weight;
            l[i]  += v * assignments.voice.front_lr_weight + d * assignments.drums.front_lr_weight + b * assignments.bass.front_lr_weight + h * assignments.harmonics.front_lr_weight + a * assignments.ambience.front_lr_weight;
            r[i]  += v * assignments.voice.front_lr_weight + d * assignments.drums.front_lr_weight + b * assignments.bass.front_lr_weight + h * assignments.harmonics.front_lr_weight + a * assignments.ambience.front_lr_weight;
            ls[i] += v * assignments.voice.rear_lr_weight + d * assignments.drums.rear_lr_weight + b * assignments.bass.rear_lr_weight + h * assignments.harmonics.rear_lr_weight + a * assignments.ambience.rear_lr_weight;
            rs[i] += v * assignments.voice.rear_lr_weight + d * assignments.drums.rear_lr_weight + b * assignments.bass.rear_lr_weight + h * assignments.harmonics.rear_lr_weight + a * assignments.ambience.rear_lr_weight;
            lfe[i]+= v * assignments.voice.lfe_weight + d * assignments.drums.lfe_weight + b * assignments.bass.lfe_weight + h * assignments.harmonics.lfe_weight + a * assignments.ambience.lfe_weight;
        }
        
        let mut stage = Self { l, r, c, ls, rs, lfe };
...
mod tests {
    ...
    fn empty_assignments() -> StemChannelAssignments {
        let zero = || ChannelAssignment { center_weight: 0.0, front_lr_weight: 0.0, rear_lr_weight: 0.0, lfe_weight: 0.0, side_weight: 0.0 };
        StemChannelAssignments {
            voice: zero(), drums: zero(), bass: zero(), harmonics: zero(), ambience: zero()
        }
    }
    ...
    fn rear_energy_within_bounds() {
        ...
        let front_energy = stage.l.iter().map(|v| v*v).sum::<f32>() + stage.r.iter().map(|v| v*v).sum::<f32>();
        let rear_energy = stage.ls.iter().map(|v| v*v).sum::<f32>() + stage.rs.iter().map(|v| v*v).sum::<f32>();
