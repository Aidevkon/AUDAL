...
impl StftEngine {
    pub fn new() -> Self {
        ...
        let window: Vec<f32> = (0..FFT_SIZE)
            .map(|n| {
                let theta = 2.0_f32 * core::f32::consts::PI * n as f32 / FFT_SIZE as f32;
    ...
    pub fn forward(&mut self, signal: &[f32]) -> (Vec<Vec<Complex<f32>>>, usize) {
        let pad = FFT_SIZE / 2;
        ...
        while pos + FFT_SIZE <= n {
            for j in 0..FFT_SIZE {
                buf[j] = Complex::new(signal[pos + j] * self.window[j], 0.0_f32);
            }
            self.fft_forward
    ...
    pub fn inverse(&mut self, frames: &[Vec<Complex<f32>>], output_len: usize) -> Vec<f32> {
        let pad = FFT_SIZE / 2;
...
impl StftStreamContext {
    ...
    pub fn forward_chunk(&mut self, chunk: &[f32]) -> Vec<Vec<f32>> {
