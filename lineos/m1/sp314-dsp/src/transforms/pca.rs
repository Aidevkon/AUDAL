...
pub fn pca_spatial(left: &[f32], right: &[f32]) -> PcaSpatialResult {
    ...
    let cov_ll: f32 = left.iter().map(|&x| (x - mean_l).powi(2)).sum::<f32>() / (n - 1.0);
    let cov_rr: f32 = right.iter().map(|&x| (x - mean_r).powi(2)).sum::<f32>() / (n - 1.0);
    let cov_lr: f32 = left
        .iter()
        .zip(right.iter())
...
mod tests {
    ...
    fn correlated_stereo_has_high_correlation() {
        ...
        assert!(
            r.correlation > 0.99,
            "Expected correlation > 0.99, got {}",
            r.correlation
        );
        assert!(
            r.pc1_ratio > 0.99,
            "Expected pc1_ratio > 0.99, got {}",
            r.pc1_ratio
        );
    ...
    fn uncorrelated_stereo_has_low_pc1_ratio() {
        ...
        let left: Vec<f32> = (0..4096).map(|i| libm::sinf(i as f32 * 0.1)).collect();
        let right: Vec<f32> = (0..4096).map(|i| libm::cosf(i as f32 * 0.1)).collect();
        let r = pca_spatial(&left, &right);
        assert!(
            r.pc1_ratio < 0.7,
    ...
    fn eigenvectors_are_orthogonal() {
        // PC1 · PC2 = 0
        let left: Vec<f32> = (0..2048).map(|i| libm::sinf(i as f32 * 0.1)).collect();
        let right: Vec<f32> = (0..2048)
            .map(|i| libm::sinf(i as f32 * 0.1 + 0.5))
            .collect();
        let r = pca_spatial(&left, &right);
        let dot = r.pc1[0] * r.pc2[0] + r.pc1[1] * r.pc2[1];
        assert!(dot.abs() < 1e-5, "PC1 and PC2 not orthogonal: dot={}", dot);
    }
}
