...
impl PostFlightVerifier {
    ...
    pub fn verify_and_trim(
        ...
        let lufs_before = measure_integrated_lufs(&audio.left, &audio.right);
        let tp_before = Self::measure_peak_dbfs(&audio.left, &audio.right);

        let delta_lufs = config.target_lufs - lufs_before;

        // Step 2: Check if already passing
        if libm::fabsf(delta_lufs) <= config.lufs_tolerance && tp_before <= config.target_tp_db {
            return VerificationResult {
        ...
        let mut trim_db = delta_lufs.clamp(-config.max_trim_db, config.max_trim_db);
        ...
        Self::apply_gain(&mut audio.left, &mut audio.right, gain_linear);

        // Step 5: Re-measure (read-only — no second trim)
        let lufs_after = measure_integrated_lufs(&audio.left, &audio.right);
        let tp_after = Self::measure_peak_dbfs(&audio.left, &audio.right);

        let passed = libm::fabsf(config.target_lufs - lufs_after) <= config.lufs_tolerance
    ...
    fn measure_peak_dbfs(left: &[f32], right: &[f32]) -> f32 {
        let max_l = left.iter().map(|s| libm::fabsf(*s)).fold(0.0_f32, f32::max);
        let max_r = right
            .iter()
            .map(|s| libm::fabsf(*s))
...
mod tests {
    ...
    fn make_buffer(left: Vec<f32>, right: Vec<f32>) -> StereoBuffer {
    ...
    fn sine_buffer(freq_hz: f32, duration_s: f32, amplitude: f32) -> StereoBuffer {
        let sr = 48000_u32;
        let n = (sr as f32 * duration_s) as usize;
        let signal: Vec<f32> = (0..n)
            .map(|i| {
                amplitude * libm::sinf(2.0 * std::f32::consts::PI * freq_hz * i as f32 / sr as f32)
            })
            .collect();
        StereoBuffer {
            left: signal.clone(),
    ...
    fn verify_pass_when_already_on_target() {
        ...
        let result = PostFlightVerifier::verify_and_trim(&mut audio, &cfg);
    ...
    fn verify_max_trim_respected() {
        ...
        let result = PostFlightVerifier::verify_and_trim(&mut audio, &cfg);
        assert!(
            result.trim_applied_db.abs() <= cfg.max_trim_db + 1e-4,
    ...
    fn verify_tp_clamp_prevents_clipping() {
        ...
        assert!(
            result.true_peak_dbfs <= cfg.target_tp_db + 0.1,
    ...
    fn verify_deterministic() {
        ...
        let r1 = PostFlightVerifier::verify_and_trim(&mut a1, &cfg);
        let r2 = PostFlightVerifier::verify_and_trim(&mut a2, &cfg);
    ...
    fn verify_one_pass_only() {
        ...
        if result.was_trimmed {
            assert_ne!(result.lufs_before_trim, result.lufs_after_trim);
        }
    }
