...
fn soft_clipper_transparent_on_low_signals() {
    ...
    assert!(
        diff < 0.005_f32,
        "Clipper altered low signal amplitude! Max out: {:.6}",
        max_out
    );
}
...
fn soft_clipper_limits_massive_peaks() {
    ...
    for i in 0..2000 {
        let t = i as f32 / 48000.0_f32;
        let sig = (t * 2.0_f32 * std::f32::consts::PI * 100.0_f32).sin() * 10.0_f32;
        let (l_out, _) = clipper.process(sig, sig);
    ...
    assert!(
        max_out <= 1.05_f32,
        "Clipper leaked massive signal! Max: {:.6}",
        max_out
    );
    assert!(
        max_out > 0.9_f32,
        "Clipper killed signal too much! Max: {:.6}",
        max_out
    );
}
