    ...
    (0..samples)
        .map(|i| {
            ...
            let env = if i < fade_samples {
                // half-cosine fade in
                0.5 * (1.0
                    - (std::f32::consts::PI * (1.0 - (i as f32 / fade_samples as f32))).cos())
            } else {
                1.0
            };
...
fn test_engine_eq_adds_energy_without_clipping() {
    ...
    assert!(output_rms > input_rms, "EQ boost must increase RMS");

    assert!(
        output_peak_db < 0.0,
        "EQ boost must not cause clipping. Peak: {} dBFS",
        output_peak_db
    );
}
...
fn test_full_pipeline_is_deterministic() {
    ...
    for i in 0..left1.len() {
        assert_eq!(left1[i], left2[i], "Full pipeline must be deterministic");
        assert_eq!(right1[i], right2[i], "Full pipeline must be deterministic");
    }
}
...
fn test_podcast_voice_ebu_r128_compliance() {
    ...
    assert!(
        output_lufs > -18.0 && output_lufs < -12.0,
        "PodcastVoice output LUFS out of EBU R128 range: {:.2} LUFS \
         (expected -18.0 to -12.0)",
        output_lufs
    );
    ...
    assert!(
        clip_ratio <= 0.01,
        "PodcastVoice clipped at -12 dBFS input. Ratio: {}",
        clip_ratio
    );
}
