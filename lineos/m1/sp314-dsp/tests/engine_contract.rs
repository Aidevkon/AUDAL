...
use sp314_dsp::pipeline::engine::{Sp314MasteringEngine, EngineConfig, calculate_adaptive_budget};
use sp314_dsp::pipeline::telemetry::{Telemetry, analyze_offline_pre_pass};
...
fn engine_telemetry_matches_reference() {
    ...
    for case in cases {
        ...
        assert!((tel.peak_db - expected_peak).abs() <= tol, "Peak DB mismatch for {}", name);
        assert!((tel.rms_db - expected_rms).abs() <= tol, "RMS DB mismatch for {}", name);
    }
}
...
fn engine_parallel_blend_at_zero_percent_bypasses_compressor() {
    ...
    assert!((left[1023] - 0.5).abs() < 1e-3, "Expected 0.5, got {}", left[1023]);
...
fn engine_limiter_prevents_clipping() {
    ...
    for i in 0..left.len() {
        if left[i].abs() > max_out { max_out = left[i].abs(); }
        if right[i].abs() > max_out { max_out = right[i].abs(); }
    }
    
    let ceiling = sp314_dsp::limiter::DEFAULT_CEILING_LINEAR;
    assert!(max_out <= ceiling + 1e-4, "Output peak {} exceeded ceiling {}", max_out, ceiling);
}
...
fn engine_limiter_is_transparent_on_quiet_signal() {
    ...
    for i in 1000..47000 { // ignore edges
