...
fn render_triplet(
    ...
    for i in 0..sig_l.len() {
        let wet_idx = i + delay;
        let w_l = if wet_idx < wet_l.len() {
            wet_l[wet_idx]
        } else {
            0.0
        };
        let w_r = if wet_idx < wet_r.len() {
            wet_r[wet_idx]
        } else {
            0.0
        };
    ...
    write_wav(
        &format!("{}_delta_aligned.wav", name),
        &delta_aligned_l,
        &delta_aligned_r,
    );
...
fn forensic_dehum() {
    ...
    for i in 0..sig_l.len() {
        ...
        let bass = (t * 55.0 * 2.0 * PI).sin() * 0.15 + (t * 110.0 * 2.0 * PI).sin() * 0.15;
...
fn forensic_combined() {
    ...
    for i in 0..sig_l.len() {
        ...
        let sibilant_burst = if t % 1.5 > 0.8 && t % 1.5 < 1.0 {
            1.0
        } else {
            0.0
        };
