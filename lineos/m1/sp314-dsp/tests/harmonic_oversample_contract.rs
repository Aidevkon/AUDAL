...
fn harmonic_no_aliasing_at_high_freq() {
    ...
    let signal: Vec<f32> = (0..n_samples)
        .map(|i| {
            let t = i as f32 / fs;
            0.5_f32 * libm::sinf(
                2.0_f32 * core::f32::consts::PI * freq * t)
        })
        .collect();
    ...
    let max_out = output[margin..].iter()
        .map(|s| s.abs())
    ...
    assert!(max_out < 1.0_f32,
        "Output exceeded bounds: {:.4}", max_out);

    // Must have signal (not silence)
    assert!(max_out > 0.01_f32,
        "Output suspiciously silent: {:.4}", max_out);
}
