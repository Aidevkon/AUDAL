...
fn hpss_masks_sum_to_unity() {
    ...
    for t in 0..n_frames { mag[t][10] = 1.0_f32; }

    // Percussive burst at frame 5
    for b in 0..n_bins { mag[5][b] = 1.0_f32; }
    ...
    println!("Checked {} active bins, max error: {:.2e}",
             checked, max_err);
    assert!(checked > 0, "No active bins found");
    assert!(max_err < 1e-5_f32,
        "Masks don't sum to 1 at active bins: {:.2e}", max_err);
}
...
fn hpss_separates_harmonic_content() {
    ...
    for bin in [50_usize, 100, 150, 200] {
        for t in 0..n_frames { mag[t][bin] = 1.0_f32; }
    }
    // Percussive frames
    for frame in [10_usize, 30, 50] {
        for b in 0..n_bins { mag[frame][b] = 1.0_f32; }
    }
    // Noise floor
...
fn hpss_separates_percussive_content() {
    ...
    let perc_mean: f32 = (0..n_bins)
        .map(|b| mask_p[10][b])
        .sum::<f32>() / n_bins as f32;

    println!("Percussive mask @ frame 10: {:.8}", perc_mean);
    println!("Expected:                   {:.8}", EXPECTED_PERC);
    assert!(perc_mean > 0.7_f32,
        "Percussive mask too weak: {:.4}", perc_mean);
}
