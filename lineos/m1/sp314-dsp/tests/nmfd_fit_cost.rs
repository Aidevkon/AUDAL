...
fn run_variant(
    ...
    for x in init_w.iter_mut() { *x = next_rand(); }
    for x in init_h.iter_mut() { *x = next_rand(); }
    ...
    let (par_w, _par_h, par_cost) = nmfd_f32(
        v,
        &init_w,
        &init_h,
        num_bins,
    ...
    if run_seq {
        println!("WFIT|name={}|frames={}|bins={}|K={}|tau={}|iters={}|seq_ms={}|par_ms={}|cost={}", 
            name, num_frames, num_bins, k, tau, num_iter, seq_ms, par_ms, par_cost);
    } else {
        println!("WFIT|name={}|frames={}|bins={}|K={}|tau={}|iters={}|par_ms={}|cost={}", 
...
fn test_nmfd_fit_cost() {
    ...
    // A: BASELINE
    run_variant("A_BASELINE", &magnitude_frames, N_BINS, num_frames, k, tau, 20, false, true);
    
    // B: BAND-COMPRESSED
    run_variant("B_BAND_COMPRESSED", &mel_v, n_mels, num_frames, k, tau, 20, false, false);
    
    // C: B + iters=12
    run_variant("C_BAND_COMPRESSED_ITERS12", &mel_v, n_mels, num_frames, k, tau, 12, false, false);
    
    // D: B + decimation x2, 20 iters
    run_variant("D_BAND_COMPRESSED_DEC2_ITERS20", &dec_mel_v, n_mels, dec_frames, k, tau, 20, false, false);
    
    // E: B + decimation x2, 12 iters (determinism assert on this one)
    run_variant("E_BAND_COMPRESSED_DEC2_ITERS12", &dec_mel_v, n_mels, dec_frames, k, tau, 12, true, false);
    
    // F: CONTROL (1025 bins, decimation x2, 12 iters)
    run_variant("F_CONTROL", &dec_v, N_BINS, dec_frames, k, tau, 12, false, false);
}
