...
fn measure_region(whole: &[f32], chunked: &[f32], start: usize, end: usize, name: &str, stem: &str) {
    ...
    println!("NMFDAUDIO|{}|stem={}|max_abs={:.6}|rms_db_delta={:.2}", name, stem, max_abs, rms_d);
}
...
fn test_jury_corrected() {
    ...
    while pos < signal.len() {
        ...
        let (chunk_h, _) = nmfd_f32_h_only(&c_v, &tensor_w, &init_h_chunk, 128, k, c_frames, tau, 12);
        
        for comp in 0..k {
            let c_mask = nmf.nmfd_component_mask_chunk(comp, &chunk_h, &tensor_w, c_frames, N_BINS, tau);
            let core_mask = if c_mask.len() > pad_frames { &c_mask[pad_frames..] } else { &[] };
    ...
    assert_eq!(unwritten_count, 0, "Harness stitch math left unwritten frames!");

    let mut a_stems = Vec::new();
    let mut b_stems = Vec::new();
    for comp in 0..k {
        a_stems.push(apply_spectral_mask_to_chunk(&full_cplx, &a_masks[comp], signal.len()));
        b_stems.push(apply_spectral_mask_to_chunk(&full_cplx, &b_masks[comp], signal.len()));
    }
    ...
    for comp in 0..k {
        let whole_mask = nmf.nmfd_component_mask_chunk(comp, &whole_h, &tensor_w, n_frames, N_BINS, tau);
        c_masks[comp] = whole_mask.clone();
        c_stems.push(apply_spectral_mask_to_chunk(&full_cplx, &whole_mask, signal.len()));
    }

    let b_boundaries: Vec<usize> = (1..=num_boundaries).map(|i| i * chunk_len).collect();
    let b_controls: Vec<usize> = (1..=num_boundaries).map(|i| i * chunk_len - chunk_len / 2).collect();
    ...
    // 1. SEAM DETECTOR
    for comp in 0..k {
        let stem = if comp == scout.voice_idx { "voice" } 
                   else if comp == scout.ambience_idx { "ambience" } 
                   else if comp == scout.bass_idx { "bass" } 
                   else if comp == scout.harmonics_idx { "harmonics" } 
                   else { "comp5" };
    ...
    for f in &a_voice_cplx {
        ...
        for b in 0..128 { avg_mel[b] += mel[b]; }
    }
    ...
    for &idx in &indices[0..16] { is_top16[idx] = true; }
    ...
    for (variant, stems) in [("A", &a_stems), ("B", &b_stems)] {
        ...
        for f in &voice_cplx {
            ...
            let mel = sp314_dsp::analysis::mel_128::fold_to_mel(&mag);
            for b in 0..128 {
                if is_top16[b] { in_band_e += mel[b]; } else { out_band_e += mel[b]; }
            }
        }
        let in_band_pct = 100.0 * in_band_e / (in_band_e + out_band_e).max(1e-9);
        println!("JURY|leakage|variant={}|stem=voice|in_band_pct={:.2}", variant, in_band_pct);
    }

    // 3. PARTITION
    let mut b_sum = vec![0.0_f32; signal.len()];
    for i in 0..signal.len() {
        for comp in 0..k { b_sum[i] += b_stems[comp][i]; }
    }
    let mut max_abs = 0.0_f32;
    let mut num = 0.0_f32;
    let mut den = 0.0_f32;
    ...
    for (variant, stems) in [("A", &a_stems), ("B", &b_stems), ("C", &c_stems)] {
        ...
        for i in 0..signal.len() {
            for comp in 0..k { s_sum[i] += stems[comp][i]; }
        }
        let ms_db = rms_db(&s_sum);
        let crest = crest_factor(&s_sum);
        println!("JURY|dynamics|variant={}|ms_db={:.2}|crest={:.2}", variant, ms_db, crest);
    }

    // 5. CONTINUITY
    for comp in 0..k {
        let stem_name = if comp == scout.voice_idx { "voice" } 
                        else if comp == scout.ambience_idx { "ambience" } 
                        else if comp == scout.bass_idx { "bass" } 
                        else if comp == scout.harmonics_idx { "harmonics" } 
                        else { "comp5" };
        ...
        let p99 = if !jumps.is_empty() { jumps[(jumps.len() as f32 * 0.99) as usize] } else { 0.0 };
        println!("JURY|continuity|stem={}|worst_jump={:.6}|natural_p99={:.6}", stem_name, worst_jump, p99);
    }
    ...
    let c0 = 48000;
    measure_region(&c_stems[scout.voice_idx], &b_stems[scout.voice_idx], c0 - 2048, c0 + 2048, "control=0", "voice");
    measure_region(&c_stems[scout.ambience_idx], &b_stems[scout.ambience_idx], c0 - 2048, c0 + 2048, "control=0", "ambience");
    ...
    for b_idx in 0..num_boundaries {
        ...
        if b_samp + 2048 <= signal.len() {
            ...
            measure_region(&c_stems[scout.voice_idx], &b_stems[scout.voice_idx], start, end, &name, "voice");
            measure_region(&c_stems[scout.ambience_idx], &b_stems[scout.ambience_idx], start, end, &name, "ambience");
        }
    }

    measure_region(&c_stems[scout.voice_idx], &b_stems[scout.voice_idx], 0, signal.len(), "global_worst", "voice");
    measure_region(&c_stems[scout.ambience_idx], &b_stems[scout.ambience_idx], 0, signal.len(), "global_worst", "ambience");
    ...
    for (variant, stems) in [("A", &a_stems), ("B", &b_stems)] {
        ...
        let in_band_pct = 100.0 * in_band_e / (in_band_e + out_band_e).max(1e-9);
        println!("JURY2|self_band|variant={}|in_band_pct={:.2}", variant, in_band_pct);
    }
    ...
    for (source_name, src_audio) in sources {
        ...
        for f in &features {
            ...
            if dec.is_speech { speech_frames += 1; }
        }
        let speech_pct = 100.0 * speech_frames as f32 / features.len().max(1) as f32;
        println!("JURY2|vad|source={}|speech_pct={:.2}", source_name, speech_pct);
    }

    // 3. ENERGY SPLIT
    for (variant, stems) in [("A", &a_stems), ("B", &b_stems)] {
        let stem_db_val = rms_db(&stems[scout.voice_idx]);
        println!("JURY2|energy|variant={}|stem_db={:.2}", variant, stem_db_val);
    }
}
