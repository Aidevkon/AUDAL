...
fn test_permutation_suspect() {
    ...
    for f in 0..n_frames {
        for b in 0..128 { full_v[b * n_frames + f] = mel_frames[f][b]; }
    }
    ...
    for comp in 0..k {
        ...
        let marker = if comp == scout.voice_idx { "<-- TARGET VOICE_IDX" } else { "" };
        println!("PERM|variant=B|comp={}|centroid_band={:.2}|h_share={:.2}|vad_speech_pct={:.2} {}", 
    ...
    for comp in 0..k {
        ...
        let marker = if comp == scout.voice_idx { "<-- TARGET VOICE_IDX" } else { "" };
        println!("PERM|variant=C|comp={}|centroid_band={:.2}|h_share={:.2}|vad_speech_pct={:.2} {}", 
            comp, centroids[comp], h_share, speech_pct, marker);
    }
}
