...
fn run_dragon_audio(
    ...
    let whole_voice_mask = nmf.nmfd_component_mask_chunk(scout.voice_idx, &whole_h, tensor_w, n_frames, N_BINS, tau);
    let whole_amb_mask = nmf.nmfd_component_mask_chunk(scout.ambience_idx, &whole_h, tensor_w, n_frames, N_BINS, tau);
    ...
    while pos < signal.len() {
        ...
        let cv_mask = nmf.nmfd_component_mask_chunk(scout.voice_idx, &chunk_h, tensor_w, c_frames, N_BINS, tau);
        let ca_mask = nmf.nmfd_component_mask_chunk(scout.ambience_idx, &chunk_h, tensor_w, c_frames, N_BINS, tau);

        let pad_frames = if pos == 0 { 0 } else { (history_len + 1024) / 512 };
        let start_frame = if pos == 0 { 0 } else { num_boundaries * (chunk_len / 512) };

        let core_cv = if cv_mask.len() > pad_frames { &cv_mask[pad_frames..] } else { &[] };
        for i in 0..core_cv.len() {
            if start_frame + i < n_frames { chunked_voice_mask[start_frame + i] = core_cv[i].clone(); }
        }
        
        let core_ca = if ca_mask.len() > pad_frames { &ca_mask[pad_frames..] } else { &[] };
        for i in 0..core_ca.len() {
            if start_frame + i < n_frames { chunked_amb_mask[start_frame + i] = core_ca[i].clone(); }
        }
    ...
    // Audio generation
    let audio_whole_voice = apply_spectral_mask_to_chunk(&full_cplx, &whole_voice_mask, signal.len());
    let audio_chunked_voice = apply_spectral_mask_to_chunk(&full_cplx, &chunked_voice_mask, signal.len());

    let audio_whole_amb = apply_spectral_mask_to_chunk(&full_cplx, &whole_amb_mask, signal.len());
    let audio_chunked_amb = apply_spectral_mask_to_chunk(&full_cplx, &chunked_amb_mask, signal.len());

    // Evaluate
    // Control region (mid chunk 0)
    let c0 = 48000;
    measure_region(&audio_whole_voice, &audio_chunked_voice, c0 - 2048, c0 + 2048, "control=0", "voice");
    measure_region(&audio_whole_amb, &audio_chunked_amb, c0 - 2048, c0 + 2048, "control=0", "ambience");
    ...
    for b_idx in 0..num_boundaries {
        ...
        if b_samp + 2048 <= signal.len() {
            ...
            measure_region(&audio_whole_voice, &audio_chunked_voice, start, end, &name, "voice");
            measure_region(&audio_whole_amb, &audio_chunked_amb, start, end, &name, "ambience");
        }
    }

    measure_region(&audio_whole_voice, &audio_chunked_voice, 0, signal.len(), "global_worst", "voice");
    measure_region(&audio_whole_amb, &audio_chunked_amb, 0, signal.len(), "global_worst", "ambience");
}
