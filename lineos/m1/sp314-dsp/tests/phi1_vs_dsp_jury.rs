...
#[test]
#[ignore = "compares Phi-1 vs legacy DSP VAD on 28 MUSDB beds and synthetic duck splice — needs /tmp/w7a/beds"]
fn test_phi1_vs_dsp_jury() {
    let beds_dir = Path::new("/tmp/w7a/beds");
    if !beds_dir.exists() {
        panic!(
            "missing fixture {}: beds are generated by \
             research/musdb-lab/w7a_vad_sep.py, which writes \
             one wav per manifest entry to /tmp/w7a/beds from \
             lineos/m1/sp314-dsp/tests/fixtures/duck_splice/manifest.json \
             and research/musdb-lab/excerpts — both in-tree. \
             it needs two release binaries built first: \
             dbus_eval/mvad_w7a and oracle_extract. see commit \
             30e1f26. thresholds here were tuned on the original \
             run and must be re-measured after a rebuild. \
             also needs /tmp/w7a/fixtures/duck_splice_synth_snr-15.flac, \
             which the same script produces in FIXTURES_DIR from in-tree flac.",
            beds_dir.display()
        );
    }
