...
fn stem_render_percussive_captures_impulse() {
    ...
    assert!(
        p_at_impulse.abs() > h_at_impulse.abs(),
        "Percussive {:.4} should exceed harmonic {:.4} at impulse",
        p_at_impulse,
        h_at_impulse
    );
    assert!(
        p_at_impulse.abs() > 0.3_f32,
        "Percussive energy too low at impulse: {:.4}",
        p_at_impulse
    );
}
...
fn stem_render_harmonic_captures_sine() {
    ...
    assert!(
        harm_energy > drum_energy,
        "Harmonic {:.4} should exceed drums {:.4}",
        harm_energy,
        drum_energy
    );
    assert!(
        harm_energy > 0.1_f32,
        "Harmonic energy too low: {:.4}",
        harm_energy
    );
}
...
fn stem_render_reconstruction_within_mastering_tolerance() {
    ...
    assert!(
        max_err < 5e-2_f32,
        "Reconstruction error too high: {:.2e}",
        max_err
    );
}
