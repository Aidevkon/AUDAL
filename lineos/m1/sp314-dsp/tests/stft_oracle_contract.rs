...
fn load_oracle() -> OracleFixture {
    ...
    let json = std::fs::read_to_string(path).expect("Run tools/oracle/stft_oracle.py first");
...
fn stft_oracle_sine_440hz_passes() {
    let oracle = load_oracle();
    assert!(
        oracle.sine_440hz.pass,
        "Python oracle sine MSE={:.2e} failed",
        oracle.sine_440hz.mse
    );
    ...
    assert!(
        mse < 1e-5,
        "Rust STFT sine MSE={:.2e} exceeds INV-OT-1 threshold 1e-5",
        mse
    );
}
...
fn stft_oracle_noise_passes() {
    let oracle = load_oracle();
    assert!(
        oracle.white_noise.pass,
        "Python oracle noise MSE={:.2e} failed",
        oracle.white_noise.mse
    );
    ...
    assert!(
        mse < 1e-5,
        "Rust STFT noise MSE={:.2e} exceeds INV-OT-1 threshold 1e-5",
        mse
    );
}
...
fn stft_fixture_sha256_locked() {
    ...
    if std::path::Path::new(lock_path).exists() {
        ...
        let actual = format!("{:x}", <sha2::Sha256 as sha2::Digest>::digest(&fixture));
