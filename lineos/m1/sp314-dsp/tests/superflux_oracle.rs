...
fn test_superflux_oracle() {
    let fixtures = vec!["bodleasons_mid", "clip_speech", "clip_podcast_st"];

    let base =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../research/superflux/outputs");
    let wav_base = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures");
    let flight_base = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../flight_clips");
    let flight_st_base =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../flight_clips_stereo");
    ...
    for fix in fixtures {
        ...
        for t in 0..test_frames {
            ...
            if t % 187 == 0 {
                println!(
                    "SFLUX|   t={:0.2}s  env_A={:.4}  env_B={:.4}  librosa={:.4}",
                    t as f32 * HOP_SIZE as f32 / 48000.0,
                    rust_env_a,
                    rust_env_b,
                    python_env
                );
        ...
        println!(
            "SFLUX| {} Gate A (E2E STFT vs librosa) - Avg Diff: {:.6}, Max Diff: {:.6}",
            fix,
            gate_a_diff / valid_frames as f64,
            max_gate_a
        );
        println!(
            "SFLUX| {} Gate B (Math only env vs librosa) - Avg Diff: {:.6}, Max Diff: {:.6}",
            fix,
            gate_b_diff / valid_frames as f64,
            max_gate_b
        );
