...
pub fn run_streaming_pipeline_with_timeline(
    ...
    if let Some(pre) = pre_analysis {
        ...
        // ΚΟΙΜΑΤΑΙ ΑΠΟ 2026-09-16.
        // ΛΟΓΟΣ: κανένα από τα οκτώ κριτήρια του
        // προορισμού δεν είναι φασματικό σχήμα. Ο κόμβος
        // αλλάζει ήχο χωρίς κριτήριο — το ίδιο που έκλεισε
        // το dehum.
        // ΜΕΤΡΗΜΕΝΟ: 18/72 μπάντες στο ταβάνι σε εννιά
        // αρχεία, διάμεσος αιτήματος 2.553 dB, μεγαλύτερο
        // 17.669 στα 50 Hz. Ο στόχος (Byrne 1994) μετρήθηκε
        // σε ελεύθερο πεδίο· ο αφηγητής ηχογραφεί κοντά.
        // ΑΚΟΥΣΤΗΚΕ: με τον πιστό παραδότη το αποτέλεσμα
        // απομακρύνεται από το πρωτότυπο. Όσο πιστότερα
        // παραδίδεται το αίτημα, τόσο χειρότερα — άρα το
        // αίτημα είναι λάθος.
        // ΞΥΠΝΑΕΙ ΟΤΑΝ: ο στόχος προκύπτει από ηχογράφηση
        // του ίδιου του αφηγητή (συνέπεια βιβλίου), όχι από
        // πληθυσμιακό μέσο όρο. Τότε οι διορθώσεις γίνονται
        // μικρές εξ ορισμού και η πιστότητα του παραδότη
        // γίνεται κέρδος.
        //
        // Ο πίνακας και τα gains του resolver παραμένουν
        // υπολογισμένα — διάγνωση, ζητήθηκε ρητά να μείνει.
        // ΔΕΝ γράφονται στους κόμβους: τα ltass_band_0..7
        // μένουν στο gain_db=0.0 της δήλωσής τους
        // (γρ. 201-238), αποδεδειγμένα
        // bit-exact passthrough μηδενικής καθυστέρησης σε
        // αυτό το gain (βλ. `ltass_sleep_tests`).
        let _compensated_gains = apply_ltass_band_compensation(ref_gains);
    }
