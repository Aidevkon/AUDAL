...
// ΔΙΟΡΘΩΣΗ 2026-09-16 (F-111): το παραπάνω ήταν
// αληθές όταν γράφτηκε (190613e, 23/07) και έπαψε
// τρεις μέρες μετά. Το b400405 (26/07) έφερε το
// κυλιόμενο ring του cepstral_flux — mfcc_prev που
// δεν μηδενίζεται ποτέ, κυλιόμενος μέσος 466
// αποστάσεων — και το σχόλιο δεν ενημερώθηκε.
//
// ΜΕΤΡΗΜΕΝΟ: το cv_ioi είναι ταυτόσημο με του
// scan_file σε κάθε παράθυρο, και στα εννιά αρχεία.
// Το cepstral_flux διαφέρει σε 98-100% των
// παραθύρων, από το πρώτο. Η διαφορά είναι μικρή —
// το b400405 τη μέτρησε στο 0.028, επτά τοις εκατό
// της απόστασης των κέντρων — και οι ετυμηγορίες
// συμφωνούν: μηδέν διαφορά σε επτά από τα δώδεκα
// δοκίμια, ένα παράθυρο στα 116 στα υπόλοιπα πέντε.
//
// ΚΑΙ ΤΑ ΔΥΟ ΔΕΝ ΣΥΓΚΡΙΝΟΝΤΑΙ ΠΟΤΕ ΣΤΗΝ ΠΑΡΑΓΩΓΗ:
// αυτό βγάζει τα όρια, το scan_file τροφοδοτεί το
// NMFD σε τριάντα δευτερόλεπτα (dsp_pipeline.rs,
// read_scout_from_raw_dump). Διαφορετική δουλειά,
// διαφορετικός καλών.
//
// ΔΕΝ ΕΝΟΠΟΙΟΥΝΤΑΙ ΣΗΜΕΡΑ: το I/O ενοποιείται, ο
// ορισμός όχι — και δεν υπάρχει δοκίμιο με γνωστή
// απάντηση για το τι χρειάζεται το NMFD.
