...
impl Default for DspState {
    fn default() -> Self {
        Self { ducking_depth: 1.0, sidechain_hold: 3,
               ms_width: 1.0, lfe_gain: 0.0 }
    }
}
...
impl AudioRepo {
    ...
    pub fn commit(&mut self, new_state: DspState, message: &str) -> String {
        ...
        self.branches.insert(self.active_branch.clone(), hash.clone());
        self.head_state.store(Arc::new(new_state));
        hash
    }

    pub fn checkout(&mut self, branch_name: &str) -> Result<(), String> {
        let hash = self.branches.get(branch_name)
            .ok_or_else(|| format!("Branch '{}' not found", branch_name))?
            .clone();
        let state = self.commits.get(&hash)
            .ok_or("Commit not found")?.state;
    ...
    pub fn revert_head(&mut self) -> Result<(), String> {
        let current_hash = self.branches
            .get(&self.active_branch)
            .ok_or("No HEAD")?.clone();
        let parent_hash = self.commits.get(&current_hash)
            .and_then(|c| c.parent_hash.clone())
            .ok_or("No parent commit — already at root")?;
        let parent_state = self.commits.get(&parent_hash)
            .ok_or("Parent commit not found")?.state;
        self.branches.insert(self.active_branch.clone(), parent_hash);
...
mod tests {
    ...
    fn test_commit_chaining() {
        ...
        let s1 = DspState { ducking_depth: 1.2, ..Default::default() };
        let s2 = DspState { ducking_depth: 1.5, ..Default::default() };
    ...
    fn test_zero_latency_checkout() {
        ...
        repo.commit(DspState { ducking_depth: 1.5, ..Default::default() },
                    "Club settings");
    ...
    fn test_revert_undo() {
        ...
        let s1 = DspState { ducking_depth: 1.5, ..Default::default() };
