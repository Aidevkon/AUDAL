...
impl UdpTelemetrySender {
    pub fn new() -> Self {
        ...
        socket.connect("127.0.0.1:9000").expect("Failed to connect UDP socket to telemetry listener");
        
        // Strictly set to non-blocking (INV-TB-1)
        socket.set_nonblocking(true).expect("Failed to set non-blocking on telemetry UDP socket");
