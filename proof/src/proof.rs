...
impl ExecutionProof {
    ...
    pub fn generate(
        ...
        let chaos_compound = format!(
            "{}:{}:{}", project_id, track_id, persona.id);
        let chaos_seed_hash = Self::sha256_hex(
            chaos_compound.as_bytes());
    ...
    fn canonicalize_json(json_str: &str) -> String {
        let value: Value = serde_json::from_str(json_str)
            .expect("valid JSON required for canonicalization");
        let sorted = Self::sort_value(value);
        serde_json::to_string(&sorted)
            .expect("re-serialization must not fail")
    }
...
mod tests {
    ...
    fn test_proof_inputs() -> (Vec<f32>, Vec<f32>,
                                PersonaConfig, DspConfig, ProofLog) {
        let input  = vec![0.1_f32, -0.2, 0.3, -0.4];
        let output = vec![0.05_f32, -0.1, 0.15, -0.2];
        (input, output, test_persona(), test_dsp_config(),
         ProofLog::new())
    }
    ...
    fn proof_generate_deterministic() {
        ...
        let c1 = ExecutionProof::generate(
            &i, &o, &p, &cfg, &log,
            "proj1", "track1", "2026-05-27T00:00:00Z", "1.0.0", "spotify");
    ...
    fn proof_different_output_different_hash() {
        ...
        let c1 = ExecutionProof::generate(
            &i, &o,  &p, &cfg, &log,
            "p","t","2026-05-27T00:00:00Z","1.0.0","spotify");
        let c2 = ExecutionProof::generate(
            &i, &o2, &p, &cfg, &log,
            "p","t","2026-05-27T00:00:00Z","1.0.0","spotify");
    ...
    fn proof_verify_correct_ok() {
        ...
        let cert = ExecutionProof::generate(
            &i, &o, &p, &cfg, &log,
            "p","t","2026-05-27T00:00:00Z","1.0.0","spotify");
        assert!(ExecutionProof::verify(
            &cert, &i, &o, &p, &cfg).is_ok());
    }
