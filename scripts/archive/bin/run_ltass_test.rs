...
fn main() {
    ...
    let res = episode_render::run(&mut source, "ltass_test", graph, -16.0, &pre, |_| Ok(())).unwrap();
    
    std::fs::copy("/tmp/m0d-mastering-ltass_test.pcm", out_path).unwrap();
