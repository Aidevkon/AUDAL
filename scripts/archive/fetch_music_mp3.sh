...
def download_files(format_str, target_ext, count_needed=15):
    query = f"mediatype:audio AND format:\"{format_str}\" AND subject:\"music\""
    query_encoded = urllib.parse.quote_plus(query)
    url = f"https://archive.org/advancedsearch.php?q={query_encoded}&fl[]=identifier&sort[]=downloads+desc&rows=20&page=1&output=json"
